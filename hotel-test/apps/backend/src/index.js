...
app.get('/health', (_q, r) => r.json({ status: 'ok', service: 'hotel-test-backend', port: PORT, agents: listAgents().length }));
app.get('/', async (_q, r) => {
  let manifest = {};
  try { manifest = JSON.parse(await fs.readFile(path.resolve(__dirname, '..', '..', '..', '..', '.hojai', 'manifest.json'), 'utf8')); } catch {}
  r.json({
    service: 'hotel-test-backend',
    project: 'Hotel Test',
    template: 'hotel',
    region: manifest.region || 'unknown',
    languages: manifest.languages || ['en'],
    agents: listAgents().map(a => a.name),
    endpoints: ['/health', '/api/room', '/api/booking', '/api/guest', '/api/admin', '/api/agents', '/api/nexha']
  });
});
...
app.listen(PORT, () => console.log(`[hotel-test-backend] listening on http://localhost:${PORT}`));
