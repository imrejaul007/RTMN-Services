...
const RENDERERS = {
  ...
  return listView(d.items || [], CARD_FNS.roomCard);
},
  "bookings": async () => {
  const d = await api('/booking');
  return listView(d.items || [], CARD_FNS.bookingCard);
},
  "agents": async () => {
  const cards = [{"name":"CEO","description":"Orchestrator. Revenue + occupancy KPIs."},{"name":"Reception","description":"Check-in / check-out, room assignment, upsell."},{"name":"Housekeeping","description":"Room status, cleaning schedule, maintenance tickets."},{"name":"Revenue","description":"Dynamic pricing, occupancy forecast, channel mix."},{"name":"Support","description":"Guest requests, complaints, concierge."}].map(a =>
