...
app.get('/health', (_q, r) => r.json({ status: 'ok', service: 'pos-test-backend', port: PORT, agents: listAgents().length }));
app.get('/', async (_q, r) => {
  let manifest = {};
  try { manifest = JSON.parse(await fs.readFile(path.resolve(__dirname, '..', '..', '..', '..', '.hojai', 'manifest.json'), 'utf8')); } catch {}
  r.json({
    service: 'pos-test-backend',
    project: 'Pos Test',
    template: 'pos',
...
app.listen(PORT, () => console.log(`[pos-test-backend] listening on http://localhost:${PORT}`));
