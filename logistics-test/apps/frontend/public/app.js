...
const RENDERERS = {
  "vehicles": async () => {
  const d = await api('/vehicle');
  return listView(d.items || [], CARD_FNS.vehicleCard);
},
  "shipments": async () => {
  const d = await api('/shipment');
  return listView(d.items || [], CARD_FNS.shipmentCard);
},
  "agents": async () => {
  const cards = [{"name":"CEO","description":"Orchestrator. Fleet utilisation, on-time delivery rate."},{"name":"Dispatch","description":"Shipment assignment, route planning, capacity matching."},{"name":"Fleet","description":"Vehicle health, maintenance schedule, driver hours."},{"name":"Customer","description":"Shipment status, ETA notifications, POD."},{"name":"Finance","description":"COD reconciliation, fuel costs, driver payouts."}].map(a =>
