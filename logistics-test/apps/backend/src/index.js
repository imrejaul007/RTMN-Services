...
app.get('/health', (_q, r) => r.json({ status: 'ok', service: 'logistics-test-backend', port: PORT, agents: listAgents().length }));
app.get('/', async (_q, r) => {
  let manifest = {};
  try { manifest = JSON.parse(await fs.readFile(path.resolve(__dirname, '..', '..', '..', '..', '.hojai', 'manifest.json'), 'utf8')); } catch {}
  r.json({
    service: 'logistics-test-backend',
    project: 'Logistics Test',
    template: 'logistics',
...
app.listen(PORT, () => console.log(`[logistics-test-backend] listening on http://localhost:${PORT}`));
