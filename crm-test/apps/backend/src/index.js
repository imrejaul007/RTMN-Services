...
app.get('/health', (_q, r) => r.json({ status: 'ok', service: 'crm-test-backend', port: PORT, agents: listAgents().length }));
app.get('/', async (_q, r) => {
  let manifest = {};
  try { manifest = JSON.parse(await fs.readFile(path.resolve(__dirname, '..', '..', '..', '..', '.hojai', 'manifest.json'), 'utf8')); } catch {}
  r.json({
    service: 'crm-test-backend',
    project: 'Crm Test',
    template: 'crm',
    region: manifest.region || 'unknown',
    languages: manifest.languages || ['en'],
    agents: listAgents().map(a => a.name),
    endpoints: ['/health', '/api/lead', '/api/deal', '/api/customer', '/api/admin', '/api/agents', '/api/nexha']
  });
});
...
app.listen(PORT, () => console.log(`[crm-test-backend] listening on http://localhost:${PORT}`));
