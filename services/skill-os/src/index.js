...
const skills = new Map();              // id -> skill record
const skillExecutions = new Map();      // id -> execution record
const skillTemplates = new Map();       // id -> template record
const skillDependencies = new Map();    // id -> dependency record
const skillEvents = [];                 // event log (bounded)
const skillMarketplace = new Map();     // third-party skills
const skillTests = new Map();           // test runs
const analytics = new Map();            // per-skill analytics rollups
const learningData = new Map();         // learning feedback per skill
const categories = new Map();           // category -> meta
const versions = new Map();             // skillId -> [version records]
const permissions = new Map();          // skillId -> [permissions]
...
app.post('/api/skills/categories', (req, res) => {
...
app.post('/api/skills/marketplace', (req, res) => {
...
app.post('/api/skills/:id/execute', async (req, res) => {
  ...
  // Runtime stub: we don't actually eval user code (security), but we simulate
  const input = req.body?.input ?? null;
  const ctx = req.body?.ctx ?? {};
  let success = true, result;
  try {
    // In a real system: new Function(s.code) and call run(input, ctx)
    result = { ok: true, skillId: s.id, version: s.version, input, ctxEcho: ctx, message: `${s.name} executed` };
  } catch (e) {
    success = false;
    result = { ok: false, error: e.message };
  }
  const exec = { id: execId, skillId: s.id, version: s.version, input, result, success, startedAt: new Date(start).toISOString(), durationMs: Date.now() - start };
  skillExecutions.set(execId, exec);
  bumpAnalytics(s.id, success, exec.durationMs);
  logEvent(s.id, success ? 'success' : 'failure', { execId, durationMs: exec.durationMs });
...
app.post('/api/skills/compose', async (req, res) => {
...
app.post('/api/skills/:id/learn', (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
...
app.post('/api/skills/:id/permissions', (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
...
app.post('/api/skill-templates', (req, res) => {
...
app.post('/api/skill-templates/:id/instantiate', (req, res) => {
...
app.post('/api/skills/:id/dependencies', (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
...
app.put('/api/skills/:id/policies', (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
...
app.post('/api/skills/:id/memory', async (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
...
app.post('/api/skills/:id/twin', async (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
...
app.post('/api/skills/:id/flow', (req, res) => {
  const s = getSkill(req.params.id);
  if (!s) return fail(res, 'NOT_FOUND', 'skill not found', 404);
