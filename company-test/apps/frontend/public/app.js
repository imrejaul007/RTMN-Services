...
const RENDERERS = {
  "employees": async () => {
  const d = await api('/employee');
  return listView(d.items || [], CARD_FNS.employeeCard);
},
  "departments": async () => {
  const d = await api('/department');
  return listView(d.items || [], CARD_FNS.departmentCard);
},
  "agents": async () => {
  const cards = [{"name":"CEO","description":"Orchestrator. Strategic decision support, KPI tracking."},{"name":"Sales","description":"Sales coaching, deal inspection, forecast."},{"name":"Marketing","description":"Campaign planning, audience insights."},{"name":"HR","description":"Hiring screen, leave approval, performance reviews."},{"name":"Finance","description":"Budget alerts, expense policy, financial summaries."},{"name":"Operations","description":"Process bottlenecks, resource allocation."}].map(a =>
