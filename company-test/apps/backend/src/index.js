...
app.get('/health', (_q, r) => r.json({ status: 'ok', service: 'company-test-backend', port: PORT, agents: listAgents().length }));
app.get('/', async (_q, r) => {
  let manifest = {};
  try { manifest = JSON.parse(await fs.readFile(path.resolve(__dirname, '..', '..', '..', '..', '.hojai', 'manifest.json'), 'utf8')); } catch {}
  r.json({
    service: 'company-test-backend',
    project: 'Company Test',
    template: 'company',
    region: manifest.region || 'unknown',
    languages: manifest.languages || ['en'],
    agents: listAgents().map(a => a.name),
    endpoints: ['/health', '/api/employee', '/api/department', '/api/payroll', '/api/admin', '/api/agents', '/api/nexha']
  });
});
...
app.listen(PORT, () => console.log(`[company-test-backend] listening on http://localhost:${PORT}`));
