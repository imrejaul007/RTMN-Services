...
## 🦷 SmileCraft Dental OS (RisaCare — external client)

> **Status:** SmileCraft Dental OS is RisaCare's dental-specific product implementation (an external client). RTMN does not own or operate it. For SmileCraft product details, see RisaCare's own repo at `companies/RisaCare/`.

RTMN connection points that SmileCraft / RisaCare may consume:

| RTMN Service | Purpose |
|--------------|---------|
| REZ-Consumer | Patient-facing app surface |
| HOJAI Clinic AI | AI scribe, ambient documentation |
| CorpPerks | Staff management |
| RIDZA | Financial intelligence |
| AssetMind | Wealth management |
| Nexha | Procurement |
| AdBazaar + BuzzLocal | Marketing |
| SUTAR GoalOS | Multi-agent orchestration |
...
## RisaCare (External Client)

**Location:** `companies/RisaCare/` (RisaCare's own repo, gitignored by RTMN)

RisaCare is an **external client** of HOJAI AI. RTMN does not own, audit, or operate RisaCare's services. For product details see RisaCare's own documentation. RTMN's role here is to:

- Provide healthcare-vertical intelligence templates (HOJAI AI service on port 4160) that RisaCare consumes as a building block
- Connect RisaCare to the RTMN Industry layer via `healthcare-os` (port 5020) when both sides agree to integration
...
## SmileCraft Dental OS — Detailed Spec (Removed)

> **Status:** This entire section was RisaCare-specific deep-dive content (Dental Twin, Dental Inventory, Imaging AI, Expansion Agent, Genie Dental, REZ Consumer Dental page, API reference, story flow). RisaCare is an external client, so RTMN does not document its internal product details. See `companies/RisaCare/` for RisaCare's own docs.
