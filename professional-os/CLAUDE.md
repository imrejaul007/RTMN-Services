# Professional OS - Development Guide

**Port:** 5170  
**Type:** Industry OS (professional)  
...
|----------|-------------|----------|
| PORT | Service port | No (default: 5170) |
...
Professional OS manages complete professional operations.
...
curl -X POST http://localhost:5170/auth/register \
  ...
  -d '{"businessId":"biz_123","email":"owner@professional.com","password":"secret"}'

# Login
curl -X POST http://localhost:5170/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"owner@professional.com","password":"secret"}'
```
