/**
 * Buyer Twin Service
 * Digital Twin service for buyer entities
 */
...
const ALLOWED_CREATE_FIELDS = ['id', 'name', 'type', 'category', 'metadata', 'status', 'email', 'phone', 'preferences', 'budget', 'location', 'agentId', 'businessId'];
const ALLOWED_UPDATE_FIELDS = ['name', 'metadata', 'status', 'email', 'phone', 'preferences', 'budget', 'location', 'agentId'];
...
// Get all buyer twins (with pagination)
...
app.post('/api/twin', strictLimiter, (req, res) => {
  try {
    ...
    const { id, name, type, category, metadata, status, email, phone, preferences, budget, location, agentId, businessId } = sanitizedBody;
...
// Start server
app.listen(PORT, () => {
  console.log(`[${SERVICE_NAME}] Buyer Twin Service running on port ${PORT}`);
