...
| **D. ADR-0010 Multi-Tenant Federation** | Phase 0-7 done (Jun 22 2026): Repo reshape, Hub de-aliasing, Event Bus, **Business Directory** (port 4360, 68 tests) + SADA public trust API (19 tests) + shared directory client (22 tests) + **ACP-Messaging** (port 4340, 78 tests) + **Agent Marketplace** (port 4250, 109 tests) + **Mission Planner** (port 4362, 120 tests) + **Partner Graph** (port 4363, 90 tests). Hub wired at `/api/nexha/nexha-business-directory/*`, `/api/nexha/nexha-acp-messaging/*`, `/api/nexha/nexha-mission-planner/*`, `/api/nexha/nexha-partner-graph/*`, `/api/sutar/marketplace-listings/*`. Phase 8-11 in progress. See [`docs/ADR/0010-MULTI-TENANT-FEDERATION.md`](docs/ADR/0010-MULTI-TENANT-FEDERATION.md) + [`docs/nexha/PHASE-LOG.md`](docs/nexha/PHASE-LOG.md). | 🔄 |
