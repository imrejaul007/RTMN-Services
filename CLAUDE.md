...
| **D. ADR-0010 Multi-Tenant Federation** | Phase 0-9 done (Jun 22 2026): Repo reshape, Hub de-aliasing, Event Bus, **Business Directory** (port 4360, 68 tests) + SADA public trust API (19 tests) + shared directory client (22 tests) + **ACP-Messaging** (port 4340, 78 tests) + **Agent Marketplace** (port 4250, 109 tests) + **Mission Planner** (port 4362, 120 tests) + **Partner Graph** (port 4363, 90 tests) + **Commerce Runtime** (port 4364, 118 tests) + **Per-Tenant SUTAR Instances** (port 4141, 110 tests = 75 service + 17 do-app + 18 REZ-Workspace). Hub wired at `/api/nexha/nexha-business-directory/*`, `/api/nexha/nexha-acp-messaging/*`, `/api/nexha/nexha-mission-planner/*`, `/api/nexha/nexha-partner-graph/*`, `/api/nexha/nexha-commerce-runtime/*`, `/api/sutar/marketplace-listings/*`, `/api/sutar/sutar-tenant-instances/*`. Phase 10-11 in progress. See [`docs/ADR/0010-MULTI-TENANT-FEDERATION.md`](docs/ADR/0010-MULTI-TENANT-FEDERATION.md) + [`docs/nexha/PHASE-LOG.md`](docs/nexha/PHASE-LOG.md). | 🔄 |
