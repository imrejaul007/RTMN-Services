...
class CircuitBreaker {
  ...
  async execute<T>(
    ...
    if (this.state === 'OPEN') {
      if (Date.now() >= this.nextAttempt) {
        ...
        logger.info([CircuitBreaker:${this.config.name}] HALF_OPEN - attempting recovery`);
      } else {
        logger.info([CircuitBreaker:${this.config.name}] OPEN - using fallback`);
        return fallback();
      }
    }
    ...
    try {
      ...
      this.recordFailure();
      logger.error([CircuitBreaker:${this.config.name}] Request failed:`, error);
      return fallback();
    }
  }
  ...
  private recordSuccess(latency: number): void {
    ...
    if (this.state === 'HALF_OPEN') {
      logger.info([CircuitBreaker:${this.config.name}] HALF_OPEN → CLOSED - recovery successful`);
      this.state = 'CLOSED';
      this.failures = 0;
    }
  }
  ...
  private recordFailure(): void {
    ...
    if (this.state === 'HALF_OPEN') {
      logger.info([CircuitBreaker:${this.config.name}] HALF_OPEN → OPEN - recovery failed`);
      this.state = 'OPEN';
      ...
      logger.info([CircuitBreaker:${this.config.name}] CLOSED → OPEN - too many failures`);
      this.state = 'OPEN';
      this.nextAttempt = Date.now() + this.config.resetTimeout;
    }
  }
  ...
  forceState(state: CircuitState): void {
    logger.info([CircuitBreaker:${this.config.name}] Forced to ${state}`);
    this.state = state;
    if (state === 'CLOSED') {
      this.failures = 0;
    }
  ...
  reset(): void {
    ...
    logger.info([CircuitBreaker:${this.config.name}] Reset`);
  }
}
