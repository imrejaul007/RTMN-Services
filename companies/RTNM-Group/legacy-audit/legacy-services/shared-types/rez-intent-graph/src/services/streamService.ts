...
export async function publishIntentEvent(event: Omit<IntentEvent, 'timestamp'>): Promise<string> {
  ...
  logger.info([Stream] Published intent event ${messageId} for user ${event.userId}`);
  return messageId || '';
}
...
export async function publishNudgeEvent(
  ...
  logger.info([Stream] Published nudge event ${event} for nudge ${nudgeId}`);
  return messageId || '';
}
...
export async function initializeConsumerGroup(stream: string): Promise<void> {
  try {
    ...
    logger.info([Stream] Created consumer group '${CONSUMER_GROUP}' for stream '${stream}'`);
  } catch (error: any) {
    if (error.message?.includes('BUSYGROUP')) {
      logger.info([Stream] Consumer group '${CONSUMER_GROUP}' already exists for stream '${stream}'`);
    } else {
      throw error;
    }
  }
...
export async function readPendingMessages(
  ...
  try {
    ...
    logger.error([Stream] Failed to read pending messages from ${stream}:`, error);
    return [];
  }
}
...
export async function readNewMessages(
  ...
  try {
    ...
    logger.error([Stream] Failed to read messages from ${stream}:`, error);
    return [];
  }
}
...
export async function acknowledgeMessages(stream: string, messageIds: string[]): Promise<void> {
  ...
  try {
    ...
    logger.info([Stream] Acknowledged ${messageIds.length} messages from ${stream}`);
  } catch (error) {
    logger.error([Stream] Failed to acknowledge messages:`, error);
  }
}
...
export async function getStreamInfo(stream: string): Promise<Record<string, any>> {
  try {
    ...
    logger.error([Stream] Failed to get stream info for ${stream}:`, error);
