...
export class DormantIntentCronJob {
  ...
  async run(): Promise<{
    ...
    try {
      ...
      logger.info([DormantIntentCron] Processed ${processed} intents, marked ${markedDormant} as dormant`);

      // Step 2: Update revival scores
      const updatedRevivals = await dormantIntentService.updateRevivalScores();
      logger.info([DormantIntentCron] Updated ${updatedRevivals} revival scores`);

      // Step 3: Get scheduled revivals and queue nudge jobs
      const scheduledNudges = await this.queueScheduledNudges();
      logger.info([DormantIntentCron] Queued ${scheduledNudges} nudge jobs`);

      // Log queue stats
      const stats = await nudgeQueue.getStats();
      logger.info([DormantIntentCron] Queue stats: ${stats.total} jobs total`);
      ...
      logger.info([DormantIntentCron] Completed in ${duration}ms`);
    ...
    for (const candidate of candidates) {
      ...
      if (candidate.revivalScore >= 0.3) {
        ...
        if (result.success) {
          ...
          queued++;
          logger.info([DormantIntentCron] Enqueued nudge for user ${candidate.dormantIntent.userId}, intent: ${candidate.dormantIntent.intentKey}`);
        }
      }
    }
  ...
  start(): void {
    ...
    logger.info([DormantIntentCron] Started with ${this.intervalMs / 1000 / 60 / 60}h interval`);
  }
