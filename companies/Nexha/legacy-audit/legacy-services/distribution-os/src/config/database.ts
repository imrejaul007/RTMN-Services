...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/nexha_distribution';
...
export async function connectDatabase(): Promise<void> {
  ...
  const retryConnection = async (): Promise<void> => {
    ...
    try {
      ...
      logger.info(`[Database] Connected to MongoDB: ${MONGODB_URI.replace(/\/\/.*@/, '//***@')}`);
      ...
      logger.error('[Database] Max retry attempts reached. Giving up.');
      throw error;
    }
  };
...
export async function disconnectDatabase(): Promise<void> {
  ...
  try {
    ...
    logger.error('[Database] Error during disconnect:', error);
    throw error;
  }
}
...
if (typeof process !== 'undefined') {
  const shutdown = async (signal: string) => {
    logger.info(`[Database] Received ${signal}. Starting graceful shutdown...`);
