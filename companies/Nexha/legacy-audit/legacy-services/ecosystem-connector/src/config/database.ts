...
/**
 * Database Configuration - EcosystemConnector (Production Ready)
 * MongoDB connection for event history with retry logic, health checks, and graceful shutdown
 */

import mongoose from 'mongoose';

const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/nexha_connector';
...
export async function connectDatabase(): Promise<void> {
  ...
  const retryConnection = async (): Promise<void> => {
    ...
    try {
      ...
      logger.info(`[Database] Connected to MongoDB: nexha_connector`);
