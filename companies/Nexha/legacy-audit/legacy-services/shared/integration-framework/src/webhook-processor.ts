...
export class WebhookProcessor {
  ...
  async processWebhook(
    ...
    try {
      ...
      // Verify signature — always required when requireSignature is true
      const secret = connection?.webhookSecret || this.defaultSecret;
      if (this.requireSignature) {
        if (!secret) {
          logger.error('[WebhookProcessor] requireSignature is true but no webhook secret configured');
          return {
            success: false,
            handlersExecuted: 0,
        ...
        if (!verification.valid) {
          logger.warn('[WebhookProcessor] Invalid webhook signature', { source, webhookPath });
          return {
            success: false,
            handlersExecuted: 0,
        ...
        logger.warn('[WebhookProcessor] Signature verification disabled — accepting unsigned webhooks', { source, webhookPath });
      }
      ...
      for (const handler of eventHandlers) {
        try {
          ...
          logger.error(`Handler ${handler.id} failed:`, error);
