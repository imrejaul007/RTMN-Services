...
export function findCarriers(filter) {
    return CARRIERS.filter((c) => {
        ...
        if (filter.country && c.regions.length > 0) {
            if (!c.regions.includes(filter.country))
                return false;
        }
        return true;
    });
 ...
 */
export function generateTrackingNumber(carrierId) {
 ...
 */
export async function bookWithCarrier(carrier, booking) {
    ...
    const deliveryTime = new Date(new Date(booking.pickupTime).getTime() + carrier.averageTransitHours * 3600 * 1000).toISOString();
    return {
        shipmentId,
        trackingNumber,
