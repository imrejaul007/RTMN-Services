...
import { registerShipment, getStatus, getShipment, cancelShipment } from '../tracking/registry.js';
const REROUTE_DELAY_THRESHOLD_HOURS = 24;
/**
 * Generate a shipment plan: routes + customs + insurance + carbon.
 */
export async function planShipment(request) {
    // 1. Generate candidate routes
    const candidates = generateCandidateRoutes(request);
    // 2. Score them
    const scored = scoreRoutes(candidates, request.optimizeFor || 'cost', request.weights);
    if (scored.length === 0) {
    ...
    const estimatedDelivery = new Date(Date.now() + recommended.totalTransitHours * 3600 * 1000).toISOString();
    return {
 ...
 */
export async function bookShipment(plan, options) {
    const pickupTime = options?.pickupTime || new Date(Date.now() + 24 * 3600 * 1000).toISOString();
    // Book each leg in parallel
    const legResults = await Promise.all(plan.recommendedRoute.legs.map(async (leg) => {
        try {
 ...
 */
export async function reroute(shipmentId, _reason) {
 ...
 */
export function cancelShipmentFn(shipmentId) {
    const shipment = getShipment(shipmentId);
    if (!shipment) {
        return { success: false, reason: 'shipment-not-found' };
    }
