 ...
 */
const HS_DUTY_RATES: Record<string, { rate: number; description: string }> = {
 ...
 */
const COUNTRY_RULES: Record<IsoCountry, {
  documents: Array<{ code: string; name: string; required: boolean }>;
 ...
 */
const SANCTIONED_DESTINATIONS: IsoCountry[] = ['KP', 'IR', 'SY', 'CU'];

function requiresSpecialPermitsFor(_category: string): string[] {
  return [];
}

const RESTRICTED_GOODS_TO: Partial<Record<IsoCountry, string[]>> = {
...
const TRADE_AGREEMENTS: Array<{
  members: IsoCountry[];
  name: string;
  dutyReduction: number; // 0.5 = 50% reduction
}> = [
...
function findTradeAgreement(origin: IsoCountry, destination: IsoCountry) {
  return TRADE_AGREEMENTS.find((a) =>
    a.members.includes(origin) && a.members.includes(destination)
  );
}
 ...
 */
export function lookupHsCode(hsCode: HsCode): { rate: number; description: string } {
 ...
 */
export function getCountryRules(country: IsoCountry) {
...
export function calculateDuties(params: {
  ...
  value: number;
  currency: string;
}): { duties: CustomsDuty[]; totalDutiesUsd: number } {
 ...
 */
export function determineDocuments(destination: IsoCountry, hsCode?: HsCode): CustomsDocument[] {
  const rules = getCountryRules(destination);
  const docs: CustomsDocument[] = rules.documents.map((d) => ({
    code: d.code,
    name: d.name,
...
export async function scheduleClearance(params: {
  ...
  destination: IsoCountry;
}): Promise<{ clearanceId: string; scheduledFor: string; status: 'scheduled' | 'pending_docs' }> {
  const missingRequired = params.documents.filter((d) => d.required).length;
  const status: 'scheduled' | 'pending_docs' = missingRequired === params.documents.length
    ? 'scheduled'
    : 'pending_docs';
