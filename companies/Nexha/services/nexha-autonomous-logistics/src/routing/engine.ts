...
export function haversineKm(
  ...
  if (a.country === b.country) return 500;
  ...
  for (const region of Object.values(sameRegion)) {
    if (region.includes(a.country) && region.includes(b.country)) return 1200;
  }
  // Cross-continental
  return 8000;
}
...
export function suggestMode(
  ...
  if (isPerishable && distanceKm > 500) return 'air';
  if (distanceKm <= 300) return 'courier';
  if (distanceKm < 1500) return 'road';
  if (distanceKm > 4000) {
    // Long-haul: sea or air based on urgency (we use air for now perishable/letters)
    if (cargoType === 'documents' || cargoType === 'electronics') return 'air';
    return 'sea';
  }
  return 'road';
}
...
function buildLeg(
  ...
  const costUsd = Math.round(
    carrier.baseRatePerKg * weightKg * Math.max(1, distanceKm / 1000) * 100
  ) / 100;

  // Carbon = grams × kg × km / 1e6 (kg)
  const carbonKg = Math.round(
    (carrier.carbonGramsPerKgKm * weightKg * distanceKm) / 1000 * 100
  ) / 100;
 ...
 */
export function buildSingleLegRoute(
  carrier: Carrier,
 ...
 */
export function buildMultiLegRoute(
  primary: Carrier,
  request: ShipmentRequest
): Route {
...
export function scoreRoutes(
  ...
  weights?: { cost?: number; speed?: number; carbon?: number; reliability?: number }
): ScoredRoute[] {
