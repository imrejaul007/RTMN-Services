/**
 * nexha-warehouse-network — Express server (port 4288).
 *
 * Two layers:
...
app.get('/health', (_req, res) => {
  res.json({ status: 'healthy', service: 'nexha-warehouse-network', port: PORT });
});
...
app.get('/api/v1/info', (_req, res) => {
  res.json({
    service: 'nexha-warehouse-network',
    version: '1.0.0',
...
const server = app.listen(PORT, () => {
  console.log(`nexha-warehouse-network running on port ${PORT}`);
