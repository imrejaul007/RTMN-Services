...
/**
 * Database Configuration - FranchiseOS (Production Ready)
 * MongoDB connection with retry logic, health checks, and graceful shutdown
 */

import mongoose from 'mongoose';

const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/nexha_franchise';
...
export async function connectDatabase(): Promise<void> {
  ...
  const retryConnection = async (): Promise<void> => {
    ...
    try {
      ...
      logger.info(`[Database] Connected to MongoDB: nexha_franchise`);
