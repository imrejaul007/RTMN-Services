/**
 * Nexha Pricing Network — Event Bus helper (ADR-0009 Phase 2).
 */
...
function getBus(): EventBus {
  if (_bus) return _bus;
  _bus = new EventBus({
    serviceName: process.env.SERVICE_NAME || 'nexha-pricing-network',
  });
  _bus.connect().catch((err) => {
    // eslint-disable-next-line no-console
    console.warn(`[nexha-pricing events] connect failed: ${err.message}`);
  });
  return _bus;
}
