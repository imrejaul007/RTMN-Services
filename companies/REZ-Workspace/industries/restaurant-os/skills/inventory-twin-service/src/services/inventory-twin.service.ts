...
export class InventoryTwinService {
  ...
  async createPurchaseOrder(inventoryId: string, request: CreatePurchaseOrderRequest): Promise<{
    ...
    // Phase 7 of NEXHA-AUDIT-V2: wire the inventory twin to procurement-os
    // so "Restaurant AI needs 500kg rice" actually fires a real RFQ.
    // The restaurant's CorpID is sourced from the inventory twin's restaurantId
    // (assumes identity-OS has linked them; in production this is via L1 commerce-identity).
    ...
    if (rfqItems.length > 0 && buyerCorpId) {
      ...
      const result = await procurementClient.createRfqFromReorder({
        ...
        description: `Auto-generated from inventory-twin reorder alerts (${inventoryTwin.reorderAlerts.length} alerts)`,
        urgency,
        items: rfqItems,
      });
