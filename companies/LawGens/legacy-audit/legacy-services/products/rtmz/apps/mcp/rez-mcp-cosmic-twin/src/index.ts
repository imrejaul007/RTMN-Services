...
const USE_REAL_API = process.env.USE_REAL_COSMIC_TWIN === 'true';
...
// Start server
async function main() {
  logger.info("REZ Cosmic Twin MCP Server running on stdio");
  logger.info(`Cosmic Twin Service URL: ${COSMIC_TWIN_SERVICE_URL}`);
  logger.info(`Real API: ${USE_REAL_API ? 'ENABLED' : 'DISABLED (set USE_REAL_COSMIC_TWIN=true to enable)'}`);
