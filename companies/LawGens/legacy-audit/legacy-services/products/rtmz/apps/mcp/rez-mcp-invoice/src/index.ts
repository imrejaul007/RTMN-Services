...
const USE_REAL_API = process.env.USE_REAL_INVOICE === 'true';
...
// Start server
async function main() {
  logger.info("REZ Invoice MCP Server running on stdio");
  logger.info(`Invoice Service URL: ${INVOICE_SERVICE_URL}`);
  logger.info(`Real API: ${USE_REAL_API ? 'ENABLED' : 'DISABLED (set USE_REAL_INVOICE=true to enable)'}`);
