...
const USE_REAL_API = process.env.USE_REAL_CONTRACT === 'true';
...
// Start server
async function main() {
  logger.info("REZ Contract Management MCP Server running on stdio");
  logger.info(`Contract Service URL: ${CONTRACT_SERVICE_URL}`);
  logger.info(`Real API: ${USE_REAL_API ? 'ENABLED' : 'DISABLED (set USE_REAL_CONTRACT=true to enable)'}`);
