...
const USE_REAL_API = process.env.USE_REAL_AUTOML === 'true';
...
// Start server
async function main() {
  logger.info("REZ AutoML MCP Server running on stdio");
  logger.info(`AutoML Service URL: ${AUTOML_SERVICE_URL}`);
  logger.info(`Real API: ${USE_REAL_API ? 'ENABLED' : 'DISABLED (set USE_REAL_AUTOML=true to enable)'}`);
