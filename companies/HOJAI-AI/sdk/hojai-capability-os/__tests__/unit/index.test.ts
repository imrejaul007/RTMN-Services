/**
 * CapabilityOS SDK - tests.
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { CapabilityOS, CapabilityOSClient, HttpError } from '../index.js';
...
test('CapabilityOS client instantiates', () => {
  const s = new CapabilityOS(baseConfig);
  assert.ok(s.capabilityOS instanceof CapabilityOSClient);
  assert.equal(s.config.apiKey, 'test-key');
});

test('info() GETs /api/v1/info', async () => {
  const m = withFetchMock(async () => jsonResponse(200, { success: true, data: { name: 'CapabilityOS' } }));
  const s = new CapabilityOS(baseConfig);
  const r = await s.capabilityOS.info();
  assert.equal(r.data.name, 'CapabilityOS');
  m.restore();
});
...
test('stats() GETs /api/v1/stats', async () => {
  ...
  const s = new CapabilityOS(baseConfig);
  const r = await s.capabilityOS.stats();
  assert.equal(r.data.count, 42);
  m.restore();
});

test('default timeout applied', () => {
  const s = new CapabilityOS({ apiKey: 'k', baseUrl: 'http://x' });
...
test('sends Authorization header when apiKey present', async () => {
  ...
  await new CapabilityOS(baseConfig).capabilityOS.info();
  m.restore();
});
...
test('CapabilityOS has at least 2 public methods', () => {
  const s = new CapabilityOS(baseConfig);
  const count = Object.getOwnPropertyNames(Object.getPrototypeOf(s.capabilityOS)).filter(n => n !== 'constructor').length;
