...
export async function request<T = unknown>(config: HojaiConfig, method: string, path: string, body?: unknown, options: { timeout?: number; maxRetries?: number; headers?: Record<string, string> } = {}): Promise<T> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      const res = await fetchImpl(url, { method, signal: controller.signal, headers: { 'Content-Type': 'application/json', ...(config.apiKey ? { Authorization: `Bearer ${config.apiKey}` } : {}), ...options.headers }, body: body !== undefined ? JSON.stringify(body) : undefined });
      clearTimeout(timer);
      if (!res.ok) {
