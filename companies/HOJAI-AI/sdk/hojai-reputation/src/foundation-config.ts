/**
 * HOJAI Reputation SDK — shared config.
 *
 * Same HojaiConfig shape used across all @hojai/* SDKs (foundation, sutar,
 * nexha, payment, etc.). Note: in reputation SDK baseUrl is optional
 * (default https://api.hojai.ai); other SDKs require it.
 */

export interface HojaiConfig {
  /** API key (issued by HOJAI Cloud) */
  apiKey?: string;
  /** Base URL for the trust hub. Defaults to https://api.hojai.ai */
  baseUrl?: string;
