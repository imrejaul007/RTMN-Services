...
export const LANGUAGES: Record<LanguageCode, UiStrings> = {
 ...
 */
export function applyI18n(rootEl: HTMLElement, strings: UiStrings): void {
  const input = rootEl.querySelector<HTMLInputElement>('.hojai-input');
  if (input) input.placeholder = strings.inputPlaceholder;
  const bubble = rootEl.querySelector<HTMLElement>('.hojai-bubble');
  if (bubble) bubble.setAttribute('aria-label', strings.openAriaLabel);
  const close = rootEl.querySelector<HTMLElement>('.hojai-close');
  if (close) close.setAttribute('aria-label', strings.closeButton);
}