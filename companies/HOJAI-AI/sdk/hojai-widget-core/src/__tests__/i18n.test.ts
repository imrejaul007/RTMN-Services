...
import { getStrings, applyI18n, LANGUAGES, type LanguageCode, type UiStrings } from '../i18n.js';
import {
  createVoiceInput,
  isVoiceSupported,
  mapLanguageToSpeech
...
test('LANGUAGES has all required languages', () => {
  const required: LanguageCode[] = ['en', 'es', 'fr', 'de', 'it', 'pt', 'zh', 'ja', 'ko', 'hi', 'ar', 'tr'];
...
test('getStrings: every bundle has all required keys', () => {
  const requiredKeys: (keyof UiStrings)[] = [
...
test('applyI18n: sets input placeholder from bundle', () => {
  const stubInput: { placeholder: string } = { placeholder: '' };
  const stubEl = {
    querySelector: (sel: string) => sel === '.hojai-input' ? stubInput : null
  };
  applyI18n(stubEl as any, getStrings('es'));
...
test('applyI18n: sets bubble aria-label from bundle', () => {
  let capturedLabel = '';
  const stubBubble = {
    setAttribute: (k: string, v: string) => { if (k === 'aria-label') capturedLabel = v; }
  };
  const stubEl = {
    querySelector: (sel: string) => sel === '.hojai-bubble' ? stubBubble : null
  };
  applyI18n(stubEl as any, getStrings('fr'));
