 ...
 */
import { getStrings, applyI18n } from './i18n.js';
...
export class HojaiWidget {
    ...
    constructor(cfg) {
        if (!cfg.apiKey)
            throw new Error('HojaiWidget: apiKey is required');
        if (!cfg.companyId)
            throw new Error('HojaiWidget: companyId is required');
    ...
    _applyI18n() {
        ...
        const input = this.rootEl.querySelector('.hojai-input');
        if (input)
            input.placeholder = this.strings.inputPlaceholder;
    }
    /**
    ...
    _wireVoice() {
        ...
        if (!this.config.voice.enabled)
            return;
        const voiceBtn = this.rootEl.querySelector('.hojai-voice');
        if (!voiceBtn)
            return;
    ...
    _startVoice() {
        ...
        this.voiceInput = createVoiceInput({
            ...
            onError: (err) => {
                ...
                const voiceBtn = this.rootEl?.querySelector('.hojai-voice');
                if (voiceBtn)
                    voiceBtn.title = this.strings.voiceError;
            }
        });
    ...
    // Internal helpers
    // ──────────────────────────────────────────────────────────────────
    async _sendUserMessage(text, options) {
        const userMsg = {
            id: this._genId(),
            role: 'user',
            content: text,
        ...
        try {
            ...
            const fallback = {
                id: this._genId(),
                role: 'assistant',
                content: this.strings.errorGeneric,
                timestamp: Date.now()
            };
    ...
    _renderRichContent(m) {
        ...
        switch (m.rich.type) {
            case 'products':
                this._renderProducts(container, m.rich);
                break;
            case 'quote':
                this._renderQuote(container, m.rich, m);
                break;
            case 'time_slots':
                this._renderTimeSlots(container, m.rich, m);
                break;
            case 'order_confirmation':
                this._renderOrderConfirmation(container, m.rich);
                break;
            case 'support_suggestions':
                this._renderSupportSuggestions(container, m.rich, m);
                break;
            case 'recommendations':
                this._renderRecommendations(container, m.rich, m);
                break;
            default:
    ...
    _renderProducts(container, rich) {
        if (rich.title) {
      ...
      `;
            const addBtn = card.querySelector('.hojai-product-add');
            if (addBtn) {
    ...
    _renderQuote(container, rich, msg) {
        const card = document.createElement('div');
    ...
    _renderTimeSlots(container, rich, msg) {
        if (rich.title) {
            const title = document.createElement('div');
            title.className = 'hojai-rich-title';
            title.textContent = rich.title;
    ...
    _renderOrderConfirmation(container, rich) {
        const card = document.createElement('div');
    ...
    _renderSupportSuggestions(container, rich, msg) {
        if (rich.title) {
            const title = document.createElement('div');
            title.className = 'hojai-rich-title';
            title.textContent = rich.title;
    ...
    _renderRecommendations(container, rich, msg) {
        if (rich.title) {
            const title = document.createElement('div');
            title.className = 'hojai-rich-title';
            title.textContent = rich.title;
      ...
      `;
            const addBtn = card.querySelector('.hojai-rec-add');
            if (addBtn) {
                addBtn.addEventListener('click', () => {
    ...
    _wireEvents() {
        if (!this.rootEl)
            return;
        const bubble = this.rootEl.querySelector('.hojai-bubble');
        const close = this.rootEl.querySelector('.hojai-close');
        const form = this.rootEl.querySelector('.hojai-form');
        const input = this.rootEl.querySelector('.hojai-input');
    ...
    _emit(event, payload) {
        this.listeners.get(event)?.forEach((fn) => {
            try {
                ...
                if (this.debug)
                    console.error('[hojai] listener error:', err);
            }
        });
    }
    _log(...args) {
        if (this.debug)
            console.log('[hojai]', ...args);
    }
    _genId() {
