...
export class NexhaInsufficientScopeError extends NexhaAuthError {
    constructor(message = "API key lacks required scope", opts) {
...
export class NexhaValidationError extends NexhaError {
    fields;
    constructor(message = "Validation failed", fields, opts) {
...
export class NexhaCircuitOpenError extends NexhaError {
    serviceName;
    retryMs;
    constructor(serviceName, retryMs) {
        super(`Circuit breaker OPEN for "${serviceName}". Retry in ${Math.ceil(retryMs / 1000)}s`, "CIRCUIT_OPEN", { retryable: true });
...
export function isRetryable(error) {
    if (error instanceof NexhaError)
