...
import { MarketOS, MarketOSClient, HttpError } from '../index.js';
...
test('MarketOS client instantiates', () => {
  const s = new MarketOS(baseConfig);
  assert.ok(s.marketOS instanceof MarketOSClient);
  assert.equal(s.config.apiKey, 'test-key');
});

test('info() GETs /api/v1/info', async () => {
  const m = withFetchMock(async () => jsonResponse(200, { success: true, data: { name: 'MarketOS' } }));
  const s = new MarketOS(baseConfig);
  const r = await s.marketOS.info();
  assert.equal(r.data.name, 'MarketOS');
  m.restore();
});
...
test('stats() GETs /api/v1/stats', async () => {
  ...
  const s = new MarketOS(baseConfig);
  const r = await s.marketOS.stats();
  assert.equal(r.data.count, 42);
  m.restore();
});

test('default timeout applied', () => {
  const s = new MarketOS({ apiKey: 'k', baseUrl: 'http://x' });
...
test('sends Authorization header when apiKey present', async () => {
  ...
  await new MarketOS(baseConfig).marketOS.info();
  m.restore();
});
...
test('MarketOS has at least 2 public methods', () => {
  const s = new MarketOS(baseConfig);
  const count = Object.getOwnPropertyNames(Object.getPrototypeOf(s.marketOS)).filter(n => n !== 'constructor').length;
