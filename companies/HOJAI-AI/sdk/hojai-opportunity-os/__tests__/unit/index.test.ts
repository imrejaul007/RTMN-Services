/**
 * OpportunityOS SDK - tests.
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { OpportunityOS, OpportunityOSClient, HttpError } from '../index.js';
...
test('OpportunityOS client instantiates', () => {
  const s = new OpportunityOS(baseConfig);
  assert.ok(s.opportunityOS instanceof OpportunityOSClient);
  assert.equal(s.config.apiKey, 'test-key');
});

test('info() GETs /api/v1/info', async () => {
  const m = withFetchMock(async () => jsonResponse(200, { success: true, data: { name: 'OpportunityOS' } }));
  const s = new OpportunityOS(baseConfig);
  const r = await s.opportunityOS.info();
  assert.equal(r.data.name, 'OpportunityOS');
  m.restore();
});
...
test('stats() GETs /api/v1/stats', async () => {
  ...
  const s = new OpportunityOS(baseConfig);
  const r = await s.opportunityOS.stats();
  assert.equal(r.data.count, 42);
  m.restore();
});

test('default timeout applied', () => {
  const s = new OpportunityOS({ apiKey: 'k', baseUrl: 'http://x' });
...
test('sends Authorization header when apiKey present', async () => {
  ...
  await new OpportunityOS(baseConfig).opportunityOS.info();
  m.restore();
});
...
test('OpportunityOS has at least 2 public methods', () => {
  const s = new OpportunityOS(baseConfig);
  const count = Object.getOwnPropertyNames(Object.getPrototypeOf(s.opportunityOS)).filter(n => n !== 'constructor').length;
