/**
 * FederationOS SDK - tests.
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { FederationOS, FederationOSClient, HttpError } from '../index.js';
...
test('FederationOS client instantiates', () => {
  const s = new FederationOS(baseConfig);
  assert.ok(s.federationOS instanceof FederationOSClient);
  assert.equal(s.config.apiKey, 'test-key');
});

test('info() GETs /api/v1/info', async () => {
  const m = withFetchMock(async () => jsonResponse(200, { success: true, data: { name: 'FederationOS' } }));
  const s = new FederationOS(baseConfig);
  const r = await s.federationOS.info();
  assert.equal(r.data.name, 'FederationOS');
  m.restore();
});
...
test('stats() GETs /api/v1/stats', async () => {
  ...
  const s = new FederationOS(baseConfig);
  const r = await s.federationOS.stats();
  assert.equal(r.data.count, 42);
  m.restore();
});

test('default timeout applied', () => {
  const s = new FederationOS({ apiKey: 'k', baseUrl: 'http://x' });
...
test('sends Authorization header when apiKey present', async () => {
  ...
  await new FederationOS(baseConfig).federationOS.info();
  m.restore();
});
...
test('FederationOS has at least 2 public methods', () => {
  const s = new FederationOS(baseConfig);
  const count = Object.getOwnPropertyNames(Object.getPrototypeOf(s.federationOS)).filter(n => n !== 'constructor').length;
