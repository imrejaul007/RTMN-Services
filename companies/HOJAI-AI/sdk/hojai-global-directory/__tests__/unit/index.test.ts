/**
 * GlobalDirectory SDK - tests.
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { GlobalDirectory, GlobalDirectoryClient, HttpError } from '../index.js';
...
test('GlobalDirectory client instantiates', () => {
  const s = new GlobalDirectory(baseConfig);
  assert.ok(s.globalDirectory instanceof GlobalDirectoryClient);
  assert.equal(s.config.apiKey, 'test-key');
});

test('info() GETs /api/v1/info', async () => {
  const m = withFetchMock(async () => jsonResponse(200, { success: true, data: { name: 'GlobalDirectory' } }));
  const s = new GlobalDirectory(baseConfig);
  const r = await s.globalDirectory.info();
  assert.equal(r.data.name, 'GlobalDirectory');
  m.restore();
});
...
test('stats() GETs /api/v1/stats', async () => {
  ...
  const s = new GlobalDirectory(baseConfig);
  const r = await s.globalDirectory.stats();
  assert.equal(r.data.count, 42);
  m.restore();
});

test('default timeout applied', () => {
  const s = new GlobalDirectory({ apiKey: 'k', baseUrl: 'http://x' });
...
test('sends Authorization header when apiKey present', async () => {
  ...
  await new GlobalDirectory(baseConfig).globalDirectory.info();
  m.restore();
});
...
test('GlobalDirectory has at least 2 public methods', () => {
  const s = new GlobalDirectory(baseConfig);
  const count = Object.getOwnPropertyNames(Object.getPrototypeOf(s.globalDirectory)).filter(n => n !== 'constructor').length;
