#!/bin/zsh
# Start all HOJAI AI services in dependency order
# Walks the new structure: products/, platform/, sutar-os/
# Usage: ./start-all.sh

LOG_DIR=/tmp/hojai-start/logs
mkdir -p "$LOG_DIR"
HOJAI_ROOT="/Users/rejaulkarim/Documents/RTMN/companies/HOJAI-AI"

STARTED=0
FAILED=0
FAILED_LIST=""
SKIPPED=0
ALREADY=0

# Service list in dependency order (foundation → infrastructure → intelligence → agents → products)
# These names match the service directory names. We find them in products/, platform/, or sutar-os/.
SERVICES=(
  # Foundation - Platform
  corpid-service api-gateway event-bus secrets-manager feature-flags context-engine
  tenant-manager graphql-federation
  # SkillOS
  skill-os
  # Infrastructure - TwinOS, MemoryOS, Twins
  memory-os memory-confidence memory-context-engine twinos-hub twinos-shared twin-memory-bridge
  voice-twin organization-twin product-twin customer-twin order-twin
  asset-twin employee-twin inventory-twin lead-twin partner-twin
  payment-twin merchant-twin user-twin wallet-twin industry-twin
  twin-capability-profile
  agent-twin area-twin buyer-twin deal-twin property-twin referral-twin
  # Intelligence
  ai-intelligence customer-intelligence journey-intelligence
  predictive-intelligence risk-intelligence decision-intelligence
  micro-intelligence
  # Copilots
  business-copilot marketing-copilot sales-copilot finance-copilot
  support-copilot executive-copilot agent-copilot
  # Data & Knowledge
  vector-db rag-platform document-intelligence graph-database knowledge-extraction
  knowledge-base knowledge-marketplace knowledge-base-service
  # Agent Cloud + ACN + SUTAR
  acp-protocol acn-hub acn-network acn-integration
  agent-analytics agent-contracts agent-learning agent-marketplace
  agent-orchestration agent-reputation agent-wallets agent-twin
  dispute-resolution negotiation-ai merchant-agents genie-shopping-agent
  agent-economy decision-engine goal-os goal-conflict-engine policy-os flow-orchestrator
  compliance-engine consent-engine
  # SUTAR OS
  sutar-gateway sutar-agent-id sutar-agent-network sutar-identity
  sutar-twin-os sutar-memory-bridge sutar-monitoring sutar-contracts
  sutar-exploration sutar-multi-agent-evaluator sutar-reputation-aggregator
  sutar-founder-os twin-marketplace discovery-engine usage-tracker
  intent-bus simulation-os roi-calculator trust-network trust-intelligence
  # Communication
  razo-keyboard live-chat whatsapp-os email-os genie-wake-word-service
  genie-listening-modes genie-device-integration
  # Training & Model
  inference-gateway prompt-manager prompt-marketplace semantic-cache model-registry
  ai-safety evaluation-harness fine-tuning-pipeline synthetic-data-generation
  gpu-cluster-manager skill-marketplace
  # Genie Products
  genie-gateway genie-companion-service genie-thinking-engine
  genie-creation-os genie-execution-engine genie-money-os genie-wellness-os
  genie-life-university genie-life-gps genie-relationship-os
  genie-briefing-service genie-calendar-service genie-memory-graph
  genie-serendipity-service genie-smart-forgetting-service genie-universal-search
  genie-learning-os genie-consultant-agent
  # Cross-cutting
  workflow-marketplace sla-manager notification-service
  onboarding-portal pilot-onboarding incident-management-service
  # Reports + Healthcare vertical
  reports-dashboard healthcare-vertical-intelligence
  # Industry solutions
  customer-support-service billing feature-flags sandbox
  # SUTAR remaining
  sutar-discovery-engine
  # Misc
  central-observability
)

# Resolve service path
resolve_path() {
  local svc=$1
  # Check products/<p>/<svc>
  for prod in products/*/; do
    if [ -d "${prod}${svc}" ]; then
      echo "${HOJAI_ROOT}/${prod}${svc}"
      return 0
    fi
  done
  # Check platform/<p>/<svc>
  for sys in platform/*/; do
    if [ -d "${sys}${svc}" ]; then
      echo "${HOJAI_ROOT}/${sys}${svc}"
      return 0
    fi
  done
  # Check sutar-os/<p>/<svc>
  for sub in sutar-os/*/; do
    if [ -d "${sub}${svc}" ]; then
      echo "${HOJAI_ROOT}/${sub}${svc}"
      return 0
    fi
  done
  return 1
}

cd "$HOJAI_ROOT"

for svc in "${SERVICES[@]}"; do
  svc_path=$(resolve_path "$svc" 2>/dev/null)
  if [ -z "$svc_path" ] || [ ! -d "$svc_path" ]; then
    SKIPPED=$((SKIPPED+1))
    continue
  fi

  if [ ! -f "$svc_path/package.json" ]; then
    SKIPPED=$((SKIPPED+1))
    continue
  fi

  # Detect port
  PORT=""
  for src in src/index.js src/index.ts src/server.js src/app.js; do
    if [ -f "$svc_path/$src" ]; then
      PORT=$(grep -oE "PORT[^a-zA-Z0-9]*\|\|[^0-9]*[0-9]{4}" "$svc_path/$src" 2>/dev/null | grep -oE "[0-9]{4}" | head -1)
      [ -n "$PORT" ] && break
    fi
  done

  # Check if already running
  if [ -n "$PORT" ] && lsof -ti :$PORT > /dev/null 2>&1; then
    ALREADY=$((ALREADY+1))
    continue
  fi

  # Try to start
  if [ ! -d "$svc_path/node_modules" ]; then
    SKIPPED=$((SKIPPED+1))
    continue
  fi

  cd "$svc_path"

  # TypeScript services need ts-node with --transpile-only (skip type errors)
  ENTRY="src/index.js"
  [ -f "dist/index.js" ] && ENTRY="dist/index.js"
  [ -f "src/index.ts" ] && [ ! -f "dist/index.js" ] && nohup npx ts-node --transpile-only src/index.ts > "$LOG_DIR/$svc.log" 2>&1 &

  if [ -f "src/index.js" ] && [ ! -f "src/index.ts" ]; then
    nohup node src/index.js > "$LOG_DIR/$svc.log" 2>&1 &
  elif [ -f "src/index.ts" ] && [ ! -f "dist/index.js" ]; then
    # Already started above
    :
  elif [ -f "dist/index.js" ]; then
    nohup node dist/index.js > "$LOG_DIR/$svc.log" 2>&1 &
  fi

  PID=$!
  sleep 0.3
  if kill -0 $PID 2>/dev/null; then
    STARTED=$((STARTED+1))
  else
    FAILED=$((FAILED+1))
    FAILED_LIST="$FAILED_LIST $svc"
  fi
  cd "$HOJAI_ROOT"
done

echo ""
echo "=== Summary ==="
echo "Started fresh: $STARTED"
echo "Already running: $ALREADY"
echo "Failed: $FAILED"
echo "Skipped: $SKIPPED"
[ -n "$FAILED_LIST" ] && echo "Failed:$FAILED_LIST"
