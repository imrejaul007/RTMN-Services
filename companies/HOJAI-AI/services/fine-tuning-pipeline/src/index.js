...
// ─── In-memory state ──────────────────────────────────────────────
const datasets = new Map();   // datasetId -> {id, name, rows, format, ...}
const jobs = new Map();       // jobId -> {id, datasetId, baseModel, config, status, ...}
const checkpoints = new Map();// checkpointId -> {id, jobId, step, metrics, ...}
