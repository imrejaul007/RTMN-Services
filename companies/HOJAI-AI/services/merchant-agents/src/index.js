...
const merchantInventory = new Map();
const merchantPricing = new Map();
const negotiationHandlers = new Map();
 ...
 */
app.post('/api/merchants/restaurant', (req, res) => {
  try {
 ...
 */
app.post('/api/merchants/hotel', (req, res) => {
  try {
    const merchant = createMerchantAI({
 ...
 */
app.post('/api/merchants/retail', (req, res) => {
  try {
    const merchant = createMerchantAI({
 ...
 */
app.post('/api/merchants/:id/message', (req, res) => {
  try {
    const merchant = merchants.get(req.params.id);
    if (!merchant) {
      return res.status(404).json({ error: 'Merchant not found' });
 ...
 */
app.put('/api/merchants/:id/orders/:orderId', (req, res) => {
  try {
    const orders = merchantOrders.get(req.params.id) || [];
 ...
 */
app.put('/api/merchants/:id/inventory', (req, res) => {
  try {
    const merchant = merchants.get(req.params.id);
    if (!merchant) {
      return res.status(404).json({ error: 'Merchant not found' });
 ...
 */
app.put('/api/merchants/:id/pricing', (req, res) => {
  try {
    const merchant = merchants.get(req.params.id);
    if (!merchant) {
      return res.status(404).json({ error: 'Merchant not found' });
