...
// In-memory storage
// =====================
const models = new Map();     // id -> model object
const versions = new Map();   // modelId -> [version objects]
...
app.patch('/api/models/:idOrSlug', (req, res) => {
  try {
    const m = findModelByIdOrSlug(req.params.idOrSlug);
    if (!m) return res.status(404).json({ error: 'MODEL_NOT_FOUND', message: 'Model not found' });
    const body = req.body || {};
...
app.delete('/api/models/:idOrSlug', (req, res) => {
  const m = findModelByIdOrSlug(req.params.idOrSlug);
  if (!m) return res.status(404).json({ error: 'MODEL_NOT_FOUND', message: 'Model not found' });
  models.delete(m.id);
...
// Versioning
// =====================
app.post('/api/models/:idOrSlug/versions', (req, res) => {
  try {
    const m = findModelByIdOrSlug(req.params.idOrSlug);
    if (!m) return res.status(404).json({ error: 'MODEL_NOT_FOUND', message: 'Model not found' });
    const body = req.body || {};
...
app.patch('/api/models/:idOrSlug/versions/:version', (req, res) => {
  try {
    const m = findModelByIdOrSlug(req.params.idOrSlug);
    if (!m) return res.status(404).json({ error: 'MODEL_NOT_FOUND', message: 'Model not found' });
    const v = findVersion(m.id, req.params.version);
...
app.post('/api/models/:idOrSlug/versions/:version/deploy', (req, res) => {
  const m = findModelByIdOrSlug(req.params.idOrSlug);
  if (!m) return res.status(404).json({ error: 'MODEL_NOT_FOUND', message: 'Model not found' });
  const list = versions.get(m.id) || [];
