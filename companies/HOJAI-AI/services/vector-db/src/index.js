 ...
 */
app.post('/api/collections/:name/vectors', (req, res) => {
  const c = collections.get(req.params.name);
  if (!c) return res.status(404).json({ error: `collection '${req.params.name}' not found`, code: 'COLLECTION_NOT_FOUND' });
 ...
 */
app.post('/api/collections/:name/vectors/batch', (req, res) => {
  const c = collections.get(req.params.name);
  if (!c) return res.status(404).json({ error: `collection '${req.params.name}' not found`, code: 'COLLECTION_NOT_FOUND' });
 ...
 */
app.delete('/api/collections/:name/vectors/:vectorId', (req, res) => {
  const c = collections.get(req.params.name);
  if (!c) return res.status(404).json({ error: `collection '${req.params.name}' not found`, code: 'COLLECTION_NOT_FOUND' });
  const v = c.vectors.get(req.params.vectorId);
  if (!v) return res.status(404).json({ error: 'vector not found', code: 'VECTOR_NOT_FOUND' });
 ...
 */
app.post('/api/collections/:name/vectors/delete-batch', (req, res) => {
  const c = collections.get(req.params.name);
  if (!c) return res.status(404).json({ error: `collection '${req.params.name}' not found`, code: 'COLLECTION_NOT_FOUND' });
  const { ids } = req.body || {};
 ...
 */
app.post('/api/collections/:name/search-by-text', (req, res) => {
  const c = collections.get(req.params.name);
  if (!c) return res.status(404).json({ error: `collection '${req.params.name}' not found`, code: 'COLLECTION_NOT_FOUND' });
