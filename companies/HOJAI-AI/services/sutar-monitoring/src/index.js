...
// ---------- Stores ----------
const services = new Map();   // id -> { id, name, url, port, lastStatus, lastProbedAt }
const probes = [];            // history: { serviceId, status, latencyMs, httpStatus, timestamp, error }
const metrics = new Map();    // serviceId -> [{ name, value, tags, timestamp }]
const alertRules = new Map(); // ruleId -> { id, name, serviceId, metric, comparator, threshold, severity, ... }
...
app.post('/api/alerts/active/:id/resolve', (req, res) => {
