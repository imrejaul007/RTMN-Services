...
const runs = new Map();        // runId -> { scenarioId, status, results, startedAt, finishedAt }
...
app.post('/api/scenarios/:id/rerun', (req, res) => {
  const scenario = scenarios.get(req.params.id);
  if (!scenario) return res.status(404).json({ error: 'Scenario not found' });
...
app.post('/api/scenarios/:id/cancel', (req, res) => {
  const scenario = scenarios.get(req.params.id);
  if (!scenario) return res.status(404).json({ error: 'Scenario not found' });
