 ...
 */
app.post('/api/contexts/:id/spans', (req, res) => {
  const ctx = contexts.get(req.params.id);
  if (!ctx) return res.status(404).json({ error: 'Context not found' });
  if (ctx.status !== 'active') {
    return res
 ...
 */
app.post('/api/contexts/:id/logs', (req, res) => {
  const ctx = contexts.get(req.params.id);
  if (!ctx) return res.status(404).json({ error: 'Context not found' });
 ...
 */
app.post('/api/contexts/lookup', (req, res) => {
 ...
 */
app.post('/api/contexts/propagate', (req, res) => {
