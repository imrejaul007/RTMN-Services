...
const twinProfiles = new Map();        // twinId -> { basicInfo, attributes, configuration, properties, dynamicFields, tags, labels }
const twinContexts = new Map();        // twinId -> { current, history[] }
const twinLifecycles = new Map();      // twinId -> { state, history[] }   (active|idle|busy|suspended|deleted|archived|pending)
const twinTimelines = new Map();       // twinId -> [ events... ]
const twinGoals = new Map();           // twinId -> [ goals... ]
const twinKnowledgeRefs = new Map();   // twinId -> [ {kind, refId, refType, refService, addedAt} ... ]
const twinCollaborations = new Map();  // twinId -> [ {partnerTwinId, kind, since, metadata} ... ]
const twinSimulations = new Map();     // twinId -> [ simulations... ]
const twinAnalytics = new Map();       // twinId -> { activity, growth, usage, relationshipHealth, ... }
const twinIdentity = new Map();        // twinId -> { corpidLink, entityType, namespace, ownership, tenant, ... }
const mergeLog = new Map();            // twinId -> { mergedFrom: [...], mergedAt, mergedBy }
