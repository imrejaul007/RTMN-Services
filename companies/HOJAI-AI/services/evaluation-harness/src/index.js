...
const benchmarks = new Map();   // id -> benchmark
const runs = new Map();         // id -> run
const comparisons = new Map();  // id -> comparison
...
app.patch('/api/benchmarks/:idOrSlug', (req, res) => {
  const b = findBenchmark(req.params.idOrSlug);
  if (!b) return res.status(404).json({ error: 'benchmark not found' });
...
app.delete('/api/benchmarks/:idOrSlug', (req, res) => {
  const b = findBenchmark(req.params.idOrSlug);
  if (!b) return res.status(404).json({ error: 'benchmark not found' });
...
app.post('/api/benchmarks/:idOrSlug/test-cases', (req, res) => {
  const b = findBenchmark(req.params.idOrSlug);
  if (!b) return res.status(404).json({ error: 'benchmark not found' });
  const c = req.body || {};
