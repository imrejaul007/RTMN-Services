...
IdentityLinkSchema.statics.linkCustomers = async function(
  ...
  const existingLink = await (this as any).findLink(masterId, linkedId);
  if (existingLink) {
...
export const IdentityLink = mongoose.model<IIdentityLink, any>("IdentityLink", IdentityLinkSchema) as any;
