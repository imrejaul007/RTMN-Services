...
app.delete('/api/breakers/:name', (req, res) => {
  const b = breakers.get(req.params.name);
...
app.patch('/api/breakers/:name/state', (req, res) => {
  const b = breakers.get(req.params.name);
  if (!b) return res.status(404).json({ error: 'BREAKER_NOT_FOUND' });
...
app.post('/api/breakers/:name/reset', (req, res) => {
  const b = breakers.get(req.params.name);
  if (!b) return res.status(404).json({ error: 'BREAKER_NOT_FOUND' });
  b.outcomes = [];
  b.stats = {
...
app.post('/api/execute/:breakerName', async (req, res) => {
...
app.post('/api/fallbacks', (req, res) => {
...
app.delete('/api/fallbacks/:name', (req, res) => {
