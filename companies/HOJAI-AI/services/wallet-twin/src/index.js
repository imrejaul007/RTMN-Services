...
const { requireAuth, preventPrototypePollution, errorHandler, notFoundHandler,
  asyncHandler, requestId, requestLogger, logger, defaultLimiter, strictLimiter
...
app.get('/api/twins/wallet/:customerId', requireAuth, defaultLimiter, asyncHandler(async (req, res) => {
  let wallet = Array.from(wallets.values()).find(w => w.customerId === req.params.customerId && w.businessId === req.user.businessId);
  if (!wallet) {
  ...
  const txns = Array.from(transactions.values()).filter(t => t.walletId === wallet.id).slice(-50);
...
app.post('/api/twins/wallet/:customerId/topup', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  if (idempotencyKey && idempotencyKeys.has(idempotencyKey)) {
    return res.json({ success: true, twin: idempotencyKeys.get(idempotencyKey), idempotent: true });
  }
  ...
  const txn = createTwin('transaction', { walletId: wallet.id, type: TX_TYPES.CREDIT, amount, source, balanceAfter: wallet.balance + wallet.credit + wallet.coins, reference: idempotencyKey });
  transactions.set(txn.id, txn);

  if (idempotencyKey) idempotencyKeys.set(idempotencyKey, wallet);

  logger.info('Wallet topped up', { walletId: wallet.id, amount, type });
...
app.post('/api/twins/wallet/:customerId/deduct', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  const txn = createTwin('transaction', { walletId: wallet.id, type: TX_TYPES.DEBIT, amount, source: reference, orderId, balanceAfter: wallet.balance + wallet.credit + wallet.coins });
  transactions.set(txn.id, txn);

  if (idempotencyKey) idempotencyKeys.set(idempotencyKey, wallet);

  logger.info('Wallet deducted', { walletId: wallet.id, amount, type });
  res.json({ success: true, twin: wallet, transaction: txn });
}));
...
app.get('/api/twins/wallet/:customerId/history', requireAuth, defaultLimiter, asyncHandler(async (req, res) => {
  ...
  let results = Array.from(transactions.values()).filter(t => t.walletId === wallet.id);
...
app.get('/api/analytics/wallet', requireAuth, asyncHandler(async (req, res) => {
  ...
  const businessWallets = Array.from(wallets.values()).filter(w => w.businessId === businessId);
...
app.listen(PORT, () => logger.info(`💰 Wallet Twin Service running on port ${PORT}`));
