/**
 * SUTAR Contract OS — Event Bus helper (ADR-0009 Phase 2).
 */
...
function getBus(): EventBus {
  if (_bus) return _bus;
  _bus = new EventBus({
    serviceName: process.env.SERVICE_NAME || 'sutar-contract-os',
  });
  _bus.connect().catch((err) => {
    // eslint-disable-next-line no-console
    console.warn(`[sutar-contract events] connect failed: ${err.message}`);
  });
  return _bus;
}
