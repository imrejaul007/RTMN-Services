#!/bin/bash
# sutar-contracts - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4185}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_contracts_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/contracts/test-id/settle" POST "/api/contracts/test-id/settle" '{"test":true}' "" "201"
