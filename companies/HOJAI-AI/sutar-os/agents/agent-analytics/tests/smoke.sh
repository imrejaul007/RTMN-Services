#!/bin/bash
# agent-analytics - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4848}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_agent_analytics_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  agent-analytics - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/events" POST "/api/events" '{"test":true}' "" "201"
run "auto POST /api/events/batch" POST "/api/events/batch" '{"test":true}' "" "201"
run "auto POST /api/dashboards" POST "/api/dashboards" '{"test":true}' "" "201"
...
run "auto GET /api/network/overview" GET "/api/network/overview" "" "" "200"
run "auto GET /api/top" GET "/api/top" "" "" "200"
