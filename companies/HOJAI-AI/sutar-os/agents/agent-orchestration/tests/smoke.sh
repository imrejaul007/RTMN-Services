#!/bin/bash
# agent-orchestration - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4851}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_agent_orchestration_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/graphs" POST "/api/graphs" '{"test":true}' "" "201"
run "auto GET /api/graphs/test-id" GET "/api/graphs/test-id" "" "" "200"
run "auto POST /api/orchestrations" POST "/api/orchestrations" '{"test":true}' "" "201"
run "auto GET /api/orchestrations/test-id" GET "/api/orchestrations/test-id" "" "" "200"
run "auto POST /api/workflows/shop-and-delive" POST "/api/workflows/shop-and-deliver" '{"test":true}' "" "201"
run "auto POST /api/workflows/compare-merchan" POST "/api/workflows/compare-merchants" '{"test":true}' "" "201"
run "auto GET /api/patterns" GET "/api/patterns" "" "" "200"
