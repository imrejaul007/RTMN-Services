#!/bin/bash
# acn-integration - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4849}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_acn_integration_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/integrate/genie-shop" POST "/api/integrate/genie-shop" '{"test":true}' "" "201"
run "auto POST /api/integrate/merchant-sync" POST "/api/integrate/merchant-sync" '{"test":true}' "" "201"
run "auto POST /api/integrate/contract-twin" POST "/api/integrate/contract-twin" '{"test":true}' "" "201"
run "auto POST /api/integrate/payment-rez" POST "/api/integrate/payment-rez" '{"test":true}' "" "201"
run "auto GET /api/integrations/health" GET "/api/integrations/health" "" "" "200"
