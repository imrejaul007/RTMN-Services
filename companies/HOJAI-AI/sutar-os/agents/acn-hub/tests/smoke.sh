#!/bin/bash
# acn-hub - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4852}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_acn_hub_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto GET /services" GET "/services" "" "" "200"
run "auto GET /info" GET "/info" "" "" "200"
run "auto POST /api/shop" POST "/api/shop" '{"test":true}' "" "201"
run "auto GET /api/overview" GET "/api/overview" "" "" "200"
run "auto GET /routes" GET "/routes" "" "" "200"
run "auto GET /status" GET "/status" "" "" "200"
run "auto GET /docs" GET "/docs" "" "" "200"
