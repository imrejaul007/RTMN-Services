#!/bin/bash
# agent-learning - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4846}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_agent_learning_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  agent-learning - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/behavior" POST "/api/behavior" '{"test":true}' "" "201"
run "auto GET /api/profile/test-user" GET "/api/profile/test-user" "" "" "200"
run "auto POST /api/negotiations" POST "/api/negotiations" '{"test":true}' "" "201"
run "auto POST /api/predict-price" POST "/api/predict-price" '{"test":true}' "" "201"
run "auto GET /api/recommendations/test-user" GET "/api/recommendations/test-user" "" "" "200"
run "auto GET /api/behavior/test-user" GET "/api/behavior/test-user" "" "" "200"
run "auto GET /api/stats" GET "/api/stats" "" "" "200"
run "auto POST /api/train" POST "/api/train" '{"test":true}' "" "201"
run "auto GET /api/models/test-id" GET "/api/models/test-id" "" "" "200"
