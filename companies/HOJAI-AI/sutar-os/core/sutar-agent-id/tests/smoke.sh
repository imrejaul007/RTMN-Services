#!/bin/bash
# sutar-agent-id - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4145}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_agent_id_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-agent-id - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/agents" GET "/api/agents" "" "" "200"
run "auto POST /api/manifest/agents-for-inten" POST "/api/manifest/agents-for-intent" '{"test":true}' "" "201"
