#!/bin/bash
# sutar-identity - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4144}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_identity_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-identity - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/identities" POST "/api/identities" '{"test":true}' "" "201"
run "auto GET /api/corpid/proxy/test-corp" GET "/api/corpid/proxy/test-corp" "" "" "200"
