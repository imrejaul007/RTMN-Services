#!/bin/bash
# sutar-gateway - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4140}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_gateway_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-gateway - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/sutar/services" GET "/api/sutar/services" "" "" "200"
run "auto GET /api/sutar/layers" GET "/api/sutar/layers" "" "" "200"
run "auto GET /api/sutar/status" GET "/api/sutar/status" "" "" "200"
