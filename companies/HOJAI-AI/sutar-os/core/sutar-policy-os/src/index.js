...
function evaluateRule(rule, context) {
  ...
  if (rule.type === 'value_limit') {
    if (context.value > rule.value) {
      return { result: rule.action || 'require_approval', reason: 'Value exceeds policy limit', requiresApproval: true };
    }
  }
...
function evaluatePolicy(policyId, context) {
  ...
  if (!matchesScope(policy.scope, context)) return { policyId, result: 'skipped', reason: 'Out of scope' };
  for (const condition of policy.conditions) {
    if (!evaluateCondition(condition, context) && condition.operator === 'required') {
...
function checkPolicies(context) {
  ...
  return { context, finalResult, policiesChecked: applicable.length, denied: results.filter(r => r.result === 'denied').length, requiresApproval: results.filter(r => r.requiresApproval).length, results };
}
...
app.get('/api/policies', requireAuth, (req, res) => {
  ...
  res.json({ total: policies.size, returned: Math.min(list.length, 100), policies: list });
});
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'sutar-policy-os', port: PORT, layer: 'Marketplace + Economy', policies: policies.size });
});
