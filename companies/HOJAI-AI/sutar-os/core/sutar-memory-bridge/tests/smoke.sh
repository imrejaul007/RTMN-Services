#!/bin/bash
# sutar-memory-bridge - Auto-generated smoke tests
set -u
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_memory_bridge_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-memory-bridge - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/memory/remember" POST "/api/memory/remember" '{"test":true}' "" "201"
run "auto POST /api/memory/recall" POST "/api/memory/recall" '{"test":true}' "" "201"
run "auto DELETE /api/memory/test-id" DELETE "/api/memory/test-id" "" "" "200"
run "auto GET /api/memory/intent-types" GET "/api/memory/intent-types" "" "" "200"
