#!/bin/bash
# sutar-agent-network - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4155}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_agent_network_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-agent-network - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/nodes" GET "/api/nodes" "" "" "200"
run "auto POST /api/edges" POST "/api/edges" '{"test":true}' "" "201"
run "auto POST /api/route" POST "/api/route" '{"test":true}' "" "201"
run "auto POST /api/messages" POST "/api/messages" '{"test":true}' "" "201"
