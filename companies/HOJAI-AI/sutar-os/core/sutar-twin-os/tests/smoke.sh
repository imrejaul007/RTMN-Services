#!/bin/bash
# sutar-twin-os - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4142}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_twin_os_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/twins/test-id/tags" POST "/api/twins/test-id/tags" '{"test":true}' "" "201"
run "auto POST /api/twins/resolve-for-intent" POST "/api/twins/resolve-for-intent" '{"test":true}' "" "201"
