...
```
GET http://localhost:4399/api/nexha/sutar-warehouse-network/api/v1/warehouses?state=MH
GET http://localhost:4399/api/nexha/sutar-warehouse-network/api/v1/stats
```

The `warehouse-network` capability in `/api/nexha/capabilities` points to `sutar-warehouse-network`.

## Run it

```bash
cd companies/HOJAI-AI/sutar-os/core/sutar-warehouse-network
npm install
