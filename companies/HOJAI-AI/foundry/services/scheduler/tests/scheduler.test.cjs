...
// Run with: NODE_ENV=test node --test tests/scheduler.test.cjs
...
process.env.DATA_DIR = '/tmp/scheduler-test';
...
describe('scheduler', () => {
  ...
  it('GET /api/jobs/:id works', async () => {
    const r = await req('GET', '/api/jobs/:id');
    assert.ok([200, 404].includes(r.status), `failed: ${r.status}`);
  });

  it('GET /api/jobs works', async () => {
    const r = await req('GET', '/api/jobs');
    assert.ok([200, 404].includes(r.status), `failed: ${r.status}`);
  });

  it('GET /api/schedules works', async () => {
    const r = await req('GET', '/api/schedules');
    assert.ok([200, 404].includes(r.status), `failed: ${r.status}`);
  });

  it('GET /api/executions works', async () => {
    const r = await req('GET', '/api/executions');
    assert.ok([200, 404].includes(r.status), `failed: ${r.status}`);
  });

  // Auth tests
  it('POST /api/jobs without token returns 401', async () => {
    const r = await req('POST', '/api/jobs', {});
    assert.equal(r.status, 401, `expected 401, got ${r.status}`);
  });

  it('POST /api/jobs with wrong token returns 401', async () => {
    const r = await req('POST', '/api/jobs', {}, { 'x-internal-token': 'wrong-token' });
    assert.equal(r.status, 401);
  });

  it('POST /api/jobs with correct token returns expected status', async () => {
    const r = await req('POST', '/api/jobs', {}, { 'x-internal-token': 'test-token' });
