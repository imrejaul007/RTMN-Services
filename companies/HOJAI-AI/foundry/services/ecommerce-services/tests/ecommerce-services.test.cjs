// Auto-generated tests for ecommerce-services
// Run with: NODE_ENV=test node --test tests/ecommerce-services.test.cjs

'use strict';

process.env.NODE_ENV = 'test';
process.env.INTERNAL_SERVICE_TOKEN = 'test-token';
process.env.PORT = '4710';
process.env.DATA_DIR = '/tmp/ecommerce-services-test';
...
describe('ecommerce-services', () => {
  ...
  it('GET /api/products/:id works', async () => {
    const r = await req('GET', '/api/products/:id');
    assert.ok([200, 404].includes(r.status), `failed: ${r.status}`);
  });
  ...
  it('POST /api/products with wrong token returns 401', async () => {
    const r = await req('POST', '/api/products', {}, { 'x-internal-token': 'wrong-token' });
    assert.equal(r.status, 401);
  });

  it('POST /api/products with correct token returns expected status', async () => {
    const r = await req('POST', '/api/products', {}, { 'x-internal-token': 'test-token' });
