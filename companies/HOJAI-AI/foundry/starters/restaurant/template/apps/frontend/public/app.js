...
const RENDERERS = {
  ...
  return listView(d.items || [], CARD_FNS.menuCard);
},
  "tables": async () => {
  const d = await api('/table');
  return listView(d.items || [], CARD_FNS.tableCard);
},
  "agents": async () => {
  const cards = [{"name":"CEO","description":"Orchestrator. Daily revenue + covers."},{"name":"Front-of-house","description":"Reservations, seating, table turnover."},{"name":"Kitchen","description":"KOT routing, prep-time, inventory checks."},{"name":"Procurement","description":"Ingredient ordering, supplier pricing, wastage tracking."},{"name":"Finance","description":"Daily reconciliation, GST, payout to delivery partners."}].map(a =>
