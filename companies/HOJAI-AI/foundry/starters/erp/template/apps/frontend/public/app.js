...
const RENDERERS = {
  ...
  return listView(d.items || [], CARD_FNS.itemCard);
},
  "pos": async () => {
  const d = await api('/po');
  return listView(d.items || [], CARD_FNS.poCard);
},
  "agents": async () => {
  const cards = [{"name":"CEO","description":"Orchestrator. Working capital, EBITDA, runway."},{"name":"Procurement","description":"Reorder suggestions, supplier scoring, RFQ drafting."},{"name":"Finance","description":"GL postings, AR/AP ageing, cashflow forecast."},{"name":"Operations","description":"Production schedule, capacity, wastage."},{"name":"HR","description":"Headcount planning, payroll, leave policy."}].map(a =>
