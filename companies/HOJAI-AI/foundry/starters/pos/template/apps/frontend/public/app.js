...
const RENDERERS = {
  ...
  return listView(d.items || [], CARD_FNS.productCard);
},
  "sales": async () => {
  const d = await api('/sale');
  return listView(d.items || [], CARD_FNS.saleCard);
},
  "agents": async () => {
  const cards = [{"name":"CEO","description":"Orchestrator. Daily takings, basket size."},{"name":"Cashier","description":"Barcode scan, totals, change, receipt."},{"name":"Inventory","description":"Reorder alerts, shrinkage, expiry tracking."},{"name":"Finance","description":"Daily reconciliation, GST split, payouts."}].map(a =>
