/**
 * {{PROJECT_TITLE}} — backend entry point.
 *
 * Marketplace backend powered by HOJAI SDKs. v0 runs in-memory
 * (no DB) so the 30-min journey boots instantly. Replace the
 * in-memory stores with Mongo/Postgres when you're ready.
 */
...
app.get('/', async (_q, r) => {
  ...
  r.json({
    ...
    endpoints: ['/health', '/api/buyer', '/api/seller', '/api/admin', '/api/agents', '/api/nexha']
  });
});
