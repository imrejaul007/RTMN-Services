#!/bin/bash
# webhook-bus - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4401}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_webhook_bus_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  webhook-bus - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /" GET "/" "" "" "200"
run "auto GET /api/event-types" GET "/api/event-types" "" "" "200"
run "auto POST /api/subscribers" POST "/api/subscribers" '{"test":true}' "" "201"
run "auto GET /api/subscribers/test-id" GET "/api/subscribers/test-id" "" "" "200"
run "auto POST /api/dispatch" POST "/api/dispatch" '{"test":true}' "" "201"
run "auto POST /api/deliveries/test-id/delive" POST "/api/deliveries/test-id/delivered" '{"test":true}' "" "201"
run "auto POST /api/deliveries/test-id/failed" POST "/api/deliveries/test-id/failed" '{"test":true}' "" "201"
run "auto POST /api/deliveries/test-id/retry" POST "/api/deliveries/test-id/retry" '{"test":true}' "" "201"
run "auto GET /api/deliveries" GET "/api/deliveries" "" "" "200"
run "auto GET /api/deliveries/test-id" GET "/api/deliveries/test-id" "" "" "200"
