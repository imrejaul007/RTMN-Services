 ...
 * Create phone verification.
 *
 * SECURITY FIX (C-11): The OTP is hashed with SHA-256+pepper before
 * storage. Only the hash is stored; the plaintext OTP is returned to
 * the caller for delivery via SMS, never persisted.
 */
export function createPhoneVerification(phone, userId = null) {
  ...
  const verification = {
    ...
    phone,
    // SECURITY: never store the plaintext OTP. Compare via hashSecret.
  ...
  // Return the plaintext OTP for SMS delivery, but it's not persisted.
 ...
 */
/**
 * Verify phone OTP using constant-time comparison against the stored hash.
 */
export function verifyPhoneOTP(phone, otp) {
  ...
  // SECURITY FIX (C-11): Use hashSecret for constant-time comparison
  // instead of plaintext comparison. Backward-compat: if the legacy
  // 'otp' field exists (e.g. seeded data), fall back to plain compare.
  let otpMatches = false;
  if (verification.otpHash) {
    otpMatches = verifySecret(otp, verification.otpHash);
  } else if (verification.otp) {
    otpMatches = timingSafeEqual(verification.otp, otp);
  }
  if (!otpMatches) {
...
export function createBusinessVerification(businessData) {
  ...
  // Simulate checks
  // SECURITY FIX (CORPID L-6 partial): guard against double-execution of the
  // background check. The setTimeout fires after a fixed delay; if the caller
  // re-triggers the verification before the timer fires (or if the timer fires
  // after the record has been deleted), the side-effects (runBusinessChecks)
  // could run against a stale or missing record. Track an in-flight flag and
  // skip if already running. A proper queue with retry semantics is the
  // long-term fix (tracked separately).
  const inFlightKey = `__inflight_business_${id}`;
  if (typeof globalThis !== 'undefined' && globalThis[inFlightKey]) {
    return verification;
  }
  if (typeof globalThis !== 'undefined') globalThis[inFlightKey] = true;
  setTimeout(() => {
    runBusinessChecks(id, businessData).finally(() => {
      if (typeof globalThis !== 'undefined') delete globalThis[inFlightKey];
    });
  }, 100);
