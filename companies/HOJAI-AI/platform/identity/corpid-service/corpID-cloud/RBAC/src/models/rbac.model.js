 ...
 *
 * SECURITY FIX (C-14): The bare '*' wildcard now expands only to
 * permissions that are NOT in the 'admin' or 'system' category.
 * System/admin permissions must be assigned explicitly. The 'cat:*'
 * form still works as before (and is now the recommended way to grant
 * broad access within a single category).
 */
export function expandWildcardPermissions(permissionList) {
  ...
  for (const perm of permissionList) {
    if (perm === '*') {
      // SECURITY: bare '*' does NOT include admin/system permissions.
      // For those, assign explicitly.
      for (const p of permissions.values()) {
        if (p.category !== 'admin' && p.category !== 'system') {
          expanded.add(p.id);
        }
      }
 ...
 * Update role.
 *
 * SECURITY FIX (C-4): The `data.allowSystemUpdate` flag check has been
 * removed from this model-level function. The service layer
 * (RoleService.update) now enforces superadmin-only access for system
 * roles by checking the caller's actual roles, not a client-supplied flag.
 *
 * Authorization is the service's responsibility, not the model's. The
 * model is a pure data function.
 */
export function updateRole(roleId, data) {
  const role = roles.get(roleId);
  if (!role) return null;

  // Drop the (now-ignored) client-supplied flag so it can't be persisted.
  const safeData = { ...data };
  delete safeData.allowSystemUpdate;
  ...
  for (const field of allowedFields) {
    if (safeData[field] !== undefined) {
      if (field === 'permissions') {
        role.permissions = expandWildcardPermissions(safeData.permissions);
        rolePermissions.set(role.id, new Set(role.permissions));
      } else {
        role[field] = safeData[field];
      }
    }
  }
