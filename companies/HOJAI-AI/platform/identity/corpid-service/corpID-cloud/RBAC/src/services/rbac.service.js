...
class RoleService {
  ...
  update(roleId, data, userId) {
    ...
    // SECURITY FIX (C-4): Instead of trusting data.allowSystemUpdate from
    // the request body, check the caller's actual roles.
    if (role.type === 'system') {
      const callerRoles = this.getUserRoles(userId);
      if (!callerRoles.includes('superadmin')) {
        throw Errors.forbidden('Only superadmin can modify system roles');
      }
    }
    // Drop the now-ignored flag so it doesn't get persisted
    delete data.allowSystemUpdate;
...
class AccessControlService {
   ...
   * Get user's roles
   *
   * SECURITY FIX (C-3): No longer derives roles from userId string prefix.
   * Looks up actual membership records across all of a user's organizations.
   * Returns the highest-privilege role found.
   */
  getUserRoles(userId) {
    if (!userId) return ['guest'];
    // Look up actual memberships (lazy import to avoid circular deps)
    try {
      const { memberships } = require('../../../organization/src/models/organization.model.js');
      const userMemberships = Array.from(memberships.values()).filter(m =>
        m.userId === userId && m.status === 'active'
      );
      if (userMemberships.length === 0) return ['guest'];
      // Map membership roles to system roles. Priority order matters.
      const roleMap = {
      ...
      for (const m of userMemberships) {
        const r = roleMap[m.role] || 'member';
        mappedRoles.add(r);
      }
      return Array.from(mappedRoles);
    } catch {
      // If org model isn't available, return guest (fail-closed)
      return ['guest'];
    }
  }
}
