#!/bin/bash
# sandbox - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4795}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sandbox_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/sandboxes" POST "/api/sandboxes" '{"test":true}' "" "201"
run "auto GET /api/sandboxes/test-id" GET "/api/sandboxes/test-id" "" "" "200"
run "auto POST /api/sandboxes/test-id/reset" POST "/api/sandboxes/test-id/reset" '{"test":true}' "" "201"
run "auto POST /api/sandboxes/test-id/extend" POST "/api/sandboxes/test-id/extend" '{"test":true}' "" "201"
run "auto POST /api/sandboxes/test-id/log" POST "/api/sandboxes/test-id/log" '{"test":true}' "" "201"
