...
const policies = new PersistentMap('policies', { serviceName: 'ai-safety' });
...
app.post('/api/check/input',requireAuth,  (req, res) => {
...
app.post('/api/check/output',requireAuth,  (req, res) => {
...
app.post('/api/check/full',requireAuth,  (req, res) => {
...
app.post('/api/pii/detect',requireAuth,  (req, res) => {
...
app.post('/api/pii/redact',requireAuth,  (req, res) => {
  const { text, options } = req.body || {};
  if (typeof text !== 'string') {
    return res.status(400).json({ error: 'text must be a string' });
  }
...
app.post('/api/injection/detect',requireAuth,  (req, res) => {
...
app.post('/api/injection/sanitize',requireAuth,  (req, res) => {
  const { prompt } = req.body || {};
  if (typeof prompt !== 'string') {
    return res.status(400).json({ error: 'prompt must be a string' });
  }
...
app.post('/api/content/score',requireAuth,  (req, res) => {
  const { text } = req.body || {};
  if (typeof text !== 'string') {
    return res.status(400).json({ error: 'text must be a string' });
  }
...
app.post('/api/hallucination/check',requireAuth,  (req, res) => {
...
app.patch('/api/policies',requireAuth,  (req, res) => {
  const body = req.body || {};
  const policy = getCurrentPolicy();
