#!/bin/bash
# usage-tracker - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4252}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_usage_tracker_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  usage-tracker - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/usage/record" POST "/api/usage/record" '{"test":true}' "" "201"
run "auto GET /api/usage" GET "/api/usage" "" "" "200"
run "auto POST /api/billing/generate" POST "/api/billing/generate" '{"test":true}' "" "201"
run "auto GET /api/billing" GET "/api/billing" "" "" "200"
run "auto GET /api/billing/test-id" GET "/api/billing/test-id" "" "" "200"
run "auto GET /api/plans" GET "/api/plans" "" "" "200"
