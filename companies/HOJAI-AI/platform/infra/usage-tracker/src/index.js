...
const plans = new PersistentMap('plans', { serviceName: 'usage-tracker' });          // planId -> { name, pricing: { metric: rate }, ... }
const quotas = new PersistentMap('quotas', { serviceName: 'usage-tracker' });         // tenantId -> { metric: limit }
const providerBalances = new PersistentMap('provider-balances', { serviceName: 'usage-tracker' }); // providerId -> { earned, pending }
...
// ---------- Usage ----------
app.post('/api/usage/record',requireAuth,  (req, res) => {
...
// ---------- Billing ----------
app.post('/api/billing/generate',requireAuth,  (req, res) => {
...
app.post('/api/quotas/:tenantId',requireAuth,  (req, res) => {
  const { tenantId } = req.params;
