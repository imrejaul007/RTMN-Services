#!/bin/bash
# feature-flags - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4745}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_feature_flags_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  feature-flags - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/flags" POST "/api/flags" '{"test":true}' "" "201"
run "auto POST /api/flags/evaluate" POST "/api/flags/evaluate" '{"test":true}' "" "201"
run "auto POST /api/flags/bulk-evaluate" POST "/api/flags/bulk-evaluate" '{"test":true}' "" "201"
