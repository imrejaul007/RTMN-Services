#!/bin/bash
# workflow-marketplace - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4370}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_workflow_marketplace_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  workflow-marketplace - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/workflows" GET "/api/workflows" "" "" "200"
run "auto GET /api/workflows/test-id" GET "/api/workflows/test-id" "" "" "200"
run "auto GET /api/categories" GET "/api/categories" "" "" "200"
run "auto GET /api/workflows/featured/list" GET "/api/workflows/featured/list" "" "" "200"
run "auto GET /api/search" GET "/api/search" "" "" "200"
run "auto GET /api/industries" GET "/api/industries" "" "" "200"
run "auto POST /api/workflows/test-id/deploy" POST "/api/workflows/test-id/deploy" '{"test":true}' "" "201"
run "auto GET /api/deployments" GET "/api/deployments" "" "" "200"
run "auto GET /api/deployments/test-id" GET "/api/deployments/test-id" "" "" "200"
run "auto POST /api/workflows/test-id/reviews" POST "/api/workflows/test-id/reviews" '{"test":true}' "" "201"
run "auto GET /api/seller/workflows" GET "/api/seller/workflows" "" "" "200"
