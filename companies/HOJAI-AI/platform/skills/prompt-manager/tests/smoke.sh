#!/bin/bash
# prompt-manager - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4771}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_prompt_manager_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  prompt-manager - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto GET /api/stats" GET "/api/stats" "" "" "200"
run "auto POST /api/templates" POST "/api/templates" '{"test":true}' "" "201"
run "auto GET /api/templates/test-idOrSlug" GET "/api/templates/test-idOrSlug" "" "" "200"
run "auto POST /api/templates/test-idOrSlug/v" POST "/api/templates/test-idOrSlug/versions" '{"test":true}' "" "201"
run "auto POST /api/templates/test-idOrSlug/r" POST "/api/templates/test-idOrSlug/rollback" '{"test":true}' "" "201"
run "auto POST /api/templates/test-idOrSlug/r" POST "/api/templates/test-idOrSlug/render" '{"test":true}' "" "201"
run "auto POST /api/render" POST "/api/render" '{"test":true}' "" "201"
run "auto POST /api/experiments" POST "/api/experiments" '{"test":true}' "" "201"
run "auto GET /api/experiments/test-id" GET "/api/experiments/test-id" "" "" "200"
run "auto POST /api/experiments/test-id/event" POST "/api/experiments/test-id/event" '{"test":true}' "" "201"
