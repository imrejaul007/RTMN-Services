#!/bin/bash
# agent-reputation - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4820}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_agent_reputation_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  agent-reputation - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/reputation" POST "/api/reputation" '{"test":true}' "" "201"
