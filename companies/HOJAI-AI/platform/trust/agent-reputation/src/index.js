...
setupSecurity(app, { serviceName: 'agent-reputation' });

const PORT = process.env.PORT || 4738;

// In-memory stores
const agentReputations = new PersistentMap('agent-reputations', { serviceName: 'agent-reputation' });
const transactionRecords = new PersistentMap('transaction-records', { serviceName: 'agent-reputation' });
const disputeRecords = new PersistentMap('dispute-records', { serviceName: 'agent-reputation' });
const trustBadges = new PersistentMap('trust-badges', { serviceName: 'agent-reputation' });
const verificationRecords = new PersistentMap('verification-records', { serviceName: 'agent-reputation' });
 ...
 */
app.post('/api/reputation/:agentId/transactions',requireAuth,  (req, res) => {
  try {
 ...
 */
app.post('/api/disputes/:disputeId/resolve',requireAuth,  (req, res) => {
  try {
 ...
 */
app.post('/api/reputation/:agentId/verify',requireAuth,  (req, res) => {
  try {
 ...
 */
app.post('/api/reputation/:agentId/block',requireAuth,  (req, res) => {
  try {
    const record = agentReputations.get(req.params.agentId);
    if (!record) {
 ...
 */
app.post('/api/reputation/:agentId/unblock',requireAuth,  (req, res) => {
  try {
    const record = agentReputations.get(req.params.agentId);
    if (!record) {
      return res.status(404).json({ error: 'Agent not found' });
