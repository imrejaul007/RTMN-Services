#!/bin/bash
# twinos-hub - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4705}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_twinos_hub_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  twinos-hub - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /auth/register" POST "/auth/register" '{"test":true}' "" "201"
run "auto POST /auth/login" POST "/auth/login" '{"test":true}' "" "201"
run "auto POST /auth/refresh" POST "/auth/refresh" '{"test":true}' "" "201"
run "auto POST /auth/logout" POST "/auth/logout" '{"test":true}' "" "201"
run "auto GET /auth/me" GET "/auth/me" "" "" "200"
run "auto GET /api/twins" GET "/api/twins" "" "" "200"
run "auto GET /api/twins/test-id" GET "/api/twins/test-id" "" "" "200"
run "auto GET /api/twins/test-id/state" GET "/api/twins/test-id/state" "" "" "200"
run "auto GET /api/relationships" GET "/api/relationships" "" "" "200"
run "auto DELETE /api/relationships/test-id" DELETE "/api/relationships/test-id" "" "" "200"
run "auto POST /api/sync/test-id" POST "/api/sync/test-id" '{"test":true}' "" "201"
