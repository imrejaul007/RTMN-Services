#!/bin/bash
# twin-capability-profile - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4470}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_twin_capability_profile_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  twin-capability-profile - Smoke"
echo "============================================"
...
run "auto GET /api/capability-graph" GET "/api/capability-graph" "" "" "200"
