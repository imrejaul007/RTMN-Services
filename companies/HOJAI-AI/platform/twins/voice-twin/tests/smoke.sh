#!/bin/bash
# voice-twin - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4876}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_voice_twin_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  voice-twin - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/voices" GET "/api/voices" "" "" "200"
run "auto GET /api/voices/test-id" GET "/api/voices/test-id" "" "" "200"
run "auto POST /api/tts" POST "/api/tts" '{"test":true}' "" "201"
run "auto POST /api/stt" POST "/api/stt" '{"test":true}' "" "201"
run "auto GET /api/profiles" GET "/api/profiles" "" "" "200"
run "auto GET /api/profiles/test-id" GET "/api/profiles/test-id" "" "" "200"
run "auto POST /api/sessions/start" POST "/api/sessions/start" '{"test":true}' "" "201"
run "auto PUT /api/sessions/test-id" PUT "/api/sessions/test-id" '{"test":true}' "" "200"
run "auto POST /api/sessions/test-id/end" POST "/api/sessions/test-id/end" '{"test":true}' "" "201"
run "auto GET /api/sessions" GET "/api/sessions" "" "" "200"
run "auto POST /api/recordings" POST "/api/recordings" '{"test":true}' "" "201"
