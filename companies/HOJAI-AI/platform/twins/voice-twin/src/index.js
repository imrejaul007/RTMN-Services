...
// Persistent storage (file-backed JSON)
const STORE_OPTS = { serviceName: 'voice-twin' };
const voices = new PersistentStore('voices', STORE_OPTS);
const profiles = new PersistentStore('profiles', STORE_OPTS);
const recordings = new PersistentStore('recordings', STORE_OPTS);
const sessions = new PersistentStore('sessions', STORE_OPTS);

// In-memory user -> profileIds index (derived from profiles)
...
app.post('/api/voices', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  await voices.set(voice.id, voice);

  // Platform integration: publish voice-created event
  publishAsync('voice.voice.created', { id: voice.id, name: voice.name });
...
app.post('/api/tts', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  // Platform integration: publish TTS-generated event
  publishAsync('voice.tts.generated', { audioId: audio.id, userId: req.user.id });
...
app.post('/api/profiles', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  platform.bridge.autoBind(profile.id, 'episodic');
  platform.memory.recordEvent('voice-profile.created', { profileId: profile.id, userId: req.user.id, name: profile.name }, profile.id);
  platform.policy.audit('create', 'voice-profile', { profileId: profile.id });
  publishAsync('voice.profile.created', { id: profile.id, userId: req.user.id, name: profile.name });
...
app.put('/api/profiles/:id', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  // Platform integration: publish profile-updated event
  publishAsync('voice.profile.updated', { id: profile.id });
...
app.delete('/api/profiles/:id', requireAuth, asyncHandler(async (req, res) => {
  ...
  // Platform integration: publish profile-deleted event
  publishAsync('voice.profile.deleted', { id: req.params.id });
...
app.post('/api/recordings', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  await recordings.set(recording.id, recording);
...
const phase5Cleanup = installPhase5(app, {
  ...
  twinType: 'voice',
  store: typeof voices !== 'undefined' ? voices : null,
  version: process.env.SERVICE_VERSION || '2.0.0',
  stats: () => ({ count: voices.size }),
})
