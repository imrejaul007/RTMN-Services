...
const inventory = new PersistentStore('inventory-items', { serviceName: 'inventory-twin' });   // Inventory items
const warehouses = new PersistentStore('warehouses', { serviceName: 'inventory-twin' });        // Warehouse locations
const transfers = new PersistentStore('transfers', { serviceName: 'inventory-twin' });         // Stock transfers
const adjustments = new PersistentStore('adjustments', { serviceName: 'inventory-twin' });     // Inventory adjustments
const suppliers = new PersistentStore('suppliers', { serviceName: 'inventory-twin' });          // Supplier management
const lowStockAlerts = new PersistentStore('low-stock-alerts', { serviceName: 'inventory-twin' }); // Low stock alerts
...
async function checkAndUpdateLowStockAlert(item) {
  ...
  if (shouldAlert && !existingAlert) {
    ...
    await lowStockAlerts.set(item.id, existingAlert);
    return { triggered: false, updated: true };
  } else if (!shouldAlert && existingAlert) {
    await lowStockAlerts.delete(item.id);
...
app.post('/api/twins/inventory',
  ...
  asyncHandler(async (req, res) => {
    ...
    await inventory.set(item.id, item);

    // Check for low stock alert
    const alertStatus = await checkAndUpdateLowStockAlert(item);

    // Platform integration
    platform.bridge.autoBind(item.id, 'episodic');
    platform.memory.recordEvent('inventory.created', { inventoryId: item.id, sku: item.sku, name: item.name }, item.id);
    platform.policy.audit('create', 'inventory', { inventoryId: item.id, sku: item.sku });
    publishAsync('inventory.item.created', { id: item.id, sku: item.sku, name: item.name });
...
app.put('/api/twins/inventory/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Persist updated record (fixes silent-mutation bug — Object.assign only mutates in-memory)
    await inventory.set(item.id, item);
    ...
    publishAsync('inventory.item.updated', { id: item.id, sku: item.sku, fields: Object.keys(req.body || {}) });
...
app.delete('/api/twins/inventory/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    publishAsync('inventory.item.deleted', { id: req.params.id });
...
app.post('/api/twins/inventory/:id/adjust',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Persist updated inventory + new adjustment (fixes silent-mutation bug for inventory)
    await Promise.all([
    ...
    // Platform integration: publish adjustment event
    publishAsync('inventory.stock.adjusted', { inventoryId: item.id, adjustmentId: adjustment.id, quantity: adjustment.quantity, reason: adjustment.reason });
...
app.post('/api/twins/inventory/:id/transfer',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Persist new transfer + updated source inventory (fixes silent-mutation bug for inventory)
    await Promise.all([
    ...
    // Check for low stock alert
    await checkAndUpdateLowStockAlert(item);

    // Platform integration: publish transfer-initiated event
    publishAsync('inventory.transfer.initiated', { transferId: transfer.id, from: transfer.fromWarehouse, to: transfer.toWarehouse, quantity: transfer.quantity });
...
app.post('/api/twins/transfers/:id/complete',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Persist updated transfer (fixes silent-mutation bug)
    await transfers.set(transfer.id, transfer);

    // Platform integration: publish transfer-completed event
    publishAsync('inventory.transfer.completed', { transferId: transfer.id, from: transfer.fromWarehouse, to: transfer.toWarehouse });
...
app.post('/api/twins/warehouses',
  ...
  asyncHandler(async (req, res) => {
    ...
    await warehouses.set(warehouse.id, warehouse);

    // Platform integration: publish warehouse-created event
    publishAsync('inventory.warehouse.created', { id: warehouse.id, code: warehouse.code, name: warehouse.name });
...
app.put('/api/twins/warehouses/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Persist updated warehouse (fixes silent-mutation bug — Object.assign only mutates in-memory)
    await warehouses.set(warehouse.id, warehouse);

    // Platform integration: publish warehouse-updated event
    publishAsync('inventory.warehouse.updated', { id: warehouse.id, fields: Object.keys(updates) });
...
app.post('/api/twins/suppliers',
  ...
  asyncHandler(async (req, res) => {
    ...
    await suppliers.set(supplier.id, supplier);

    // Platform integration: publish supplier-created event
    publishAsync('inventory.supplier.created', { id: supplier.id, code: supplier.code, name: supplier.name });
...
const phase5Cleanup = installPhase5(app, {
  ...
  twinType: 'inventory',
  store: typeof inventory !== 'undefined' ? inventory : null,
  version: process.env.SERVICE_VERSION || '2.0.0',
  stats: () => ({ count: inventory.size }),
})
