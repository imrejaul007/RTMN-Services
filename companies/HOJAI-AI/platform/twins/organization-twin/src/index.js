...
import { optionalAuth } from '@rtmn/twinos-shared/src/middleware/auth.js';
...
const STORE_OPTS = { serviceName: 'organization-twin' };
const organizations = new PersistentStore('organizations', STORE_OPTS);
const departments   = new PersistentStore('departments',   STORE_OPTS);
const locations     = new PersistentStore('locations',     STORE_OPTS);
const relationships = new PersistentStore('relationships', STORE_OPTS);
...
// Track writes so we can await them after listen (so seeds persist to disk)
...
app.post('/api/organizations', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  await organizations.set(org.id, org);

  // Platform integration: bind to memory, record event, audit, publish (all non-blocking)
  platform.bridge.autoBind(org.id, 'episodic');
  platform.memory.recordEvent('organization.created', { orgId: org.id, name: org.name, industry: org.industry }, org.id);
  platform.policy.audit('create', 'organization', { orgId: org.id, name: org.name });
  publishAsync('organization.organization.created', { id: org.id, name: org.name, industry: org.industry, status: org.status });
...
app.put('/api/organizations/:id', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  // Platform integration: record event + publish (non-blocking)
  platform.memory.recordEvent('organization.updated', { orgId: org.id, changes: Object.keys(updates) }, org.id);
  publishAsync('organization.organization.updated', { id: org.id, changes: Object.keys(updates) });
...
app.delete('/api/organizations/:id', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  for (const [deptId, dept] of departments.entries()) {
    if (dept.orgId === orgId) deptIdsToDelete.push(deptId);
  }
  await Promise.all(deptIdsToDelete.map(id => departments.delete(id)));

  const relIdsToDelete = [];
  for (const [relId, rel] of relationships.entries()) {
    if (rel.fromOrgId === orgId || rel.toOrgId === orgId) relIdsToDelete.push(relId);
  }
  await Promise.all(relIdsToDelete.map(id => relationships.delete(id)));

  // Platform integration: unbind memory, record event, publish (non-blocking)
  platform.bridge.unbind(orgId);
  platform.memory.recordEvent('organization.deleted', { orgId }, orgId);
  publishAsync('organization.organization.deleted', { id: orgId });
...
app.post('/api/organizations/:id/departments', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  await departments.set(dept.id, dept);

  // Platform integration: record event + publish (non-blocking)
  platform.memory.recordEvent('organization.department-created', { orgId: org.id, deptId: dept.id, name: dept.name }, org.id);
  publishAsync('organization.department.created', { orgId: org.id, deptId: dept.id, name: dept.name });
...
app.put('/api/organizations/:id/health', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  // Platform integration: record health event + publish (non-blocking)
  platform.memory.recordEvent('organization.health-updated', { orgId: org.id, metrics: Object.keys(sanitizedBody).filter(k => VALID_HEALTH_METRICS.includes(k)) }, org.id);
  publishAsync('organization.health.updated', { id: org.id, health: org.health });
...
app.put('/api/organizations/:id/kpis', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  // Platform integration: record KPI progress + publish (non-blocking)
  platform.goals.recordProgress('org-kpis', org.kpis.revenue, { orgId: org.id });
  publishAsync('organization.kpis.updated', { id: org.id, kpis: org.kpis });
...
app.post('/api/organizations/:id/relationships', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  await relationships.set(relationship.id, relationship);

  // Platform integration: publish relationship creation (non-blocking)
  publishAsync('organization.relationship.created', { orgId: req.params.id, toOrgId, type: relationship.type });
...
app.delete('/api/relationships/:id', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  await relationships.delete(req.params.id);
...
app.post('/api/organizations/:id/sync', strictLimiter, requireAuth, asyncHandler(async (req, res) => {
  ...
  await syncEvents.set(event.id, event);

  // Update organization timestamp
  org.updatedAt = new Date().toISOString();
  await organizations.set(org.id, org);

  // Platform integration: publish sync completion (non-blocking)
  publishAsync('organization.sync.completed', { id: org.id, syncId: event.id });
...
const phase5Cleanup = installPhase5(app, {
  ...
  store: typeof organizations !== 'undefined' ? organizations : null,
  version: process.env.SERVICE_VERSION || '2.0.0',
  stats: () => ({ count: organizations.size }),
})
