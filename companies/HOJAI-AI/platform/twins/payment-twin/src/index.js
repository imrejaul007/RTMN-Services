...
const payments = new PersistentStore('payments', { serviceName: 'payment-twin' });
const refunds = new PersistentStore('refunds', { serviceName: 'payment-twin' });
const chargebacks = new PersistentStore('chargebacks', { serviceName: 'payment-twin' });
const payouts = new PersistentStore('payouts', { serviceName: 'payment-twin' });

// Idempotency tracking for operations (TTL cache, not a data store)
...
app.post('/api/twins/payments', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  platform.bridge.autoBind(payment.id, 'transactional');
  platform.memory.recordEvent('payment.created', { paymentId: payment.id, amount, currency: payment.currency, customerId }, payment.id);
  platform.policy.audit('create', 'payment', { paymentId: payment.id, amount });
  publishAsync('payment.payment.created', { id: payment.id, amount, currency: payment.currency, status: payment.status });
...
app.post('/api/twins/payment/:id/refund', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  await refunds.set(refund.id, refund);
  ...
  // Platform integration: publish refund event (non-blocking)
  publishAsync('payment.refund.created', { paymentId: payment.id, refundId: refund.id, amount: refundAmount });
 ...
 */
app.post('/api/twins/chargeback',requireAuth,  strictLimiter, asyncHandler(async (req, res) => {
  ...
  await chargebacks.set(chargeback.id, chargeback);

  // Platform integration: publish chargeback event (non-blocking)
  publishAsync('payment.chargeback.created', { paymentId, chargebackId: chargeback.id });
 ...
 */
app.put('/api/twins/chargeback/:id/resolve',requireAuth,  strictLimiter, asyncHandler(async (req, res) => {
...
app.post('/api/twins/payout', requireAuth, strictLimiter, asyncHandler(async (req, res) => {
  ...
  await payouts.set(payout.id, payout);
  ...
  // Platform integration: publish payout event (non-blocking)
  publishAsync('payment.payout.created', { payoutId: payout.id, merchantId: req.user.businessId, amount });
...
const phase5Cleanup = installPhase5(app, {
  ...
  twinType: 'payment',
  store: typeof payments !== 'undefined' ? payments : null,
  version: process.env.SERVICE_VERSION || '2.0.0',
  stats: () => ({ count: payments.size }),
})
