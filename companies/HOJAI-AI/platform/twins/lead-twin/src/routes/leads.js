...
import { leads, activities } from '../services/store.js';
...
router.post('/', async (req, res, next) => {
  try {
    ...
    await leads.set(id, lead);

    // Platform integration
    platform.bridge.autoBind(id, 'episodic');
    platform.memory.recordEvent('lead.created', { leadId: id, name: lead.name, email: lead.email, status: lead.status }, id);
    platform.policy.audit('create', 'lead', { leadId: id });
    publishAsync('lead.lead.created', { id, name: lead.name, status: lead.status });
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    await leads.set(id, updated);

    // Platform integration: publish update event
    publishAsync('lead.lead.updated', { id, fields: Object.keys(updates) });
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    // Collect activity IDs to cascade-delete BEFORE removing the lead,
    // then delete the lead and activities in parallel.
    const orphanActivityIds = Array.from(activities.values())
      .filter(a => a.leadId === id)
      .map(a => a.id);

    await leads.delete(id);
    await Promise.all(orphanActivityIds.map(aid => activities.delete(aid)));

    // Platform integration: publish delete event
    publishAsync('lead.lead.deleted', { id });

    logger.info('Lead deleted', {
      leadId: id,
      userId: req.user?.id,
      requestId: req.id,
      cascadedActivities: orphanActivityIds.length
    });
