 ...
 * File-backed JSON storage for leads and activities with shared access
 */
...
// ============ SHARED STORAGE ============
// File-backed JSON stores (survive process restarts).

export const leads = new PersistentStore('leads', { serviceName: 'lead-twin' });
export const activities = new PersistentStore('activities', { serviceName: 'lead-twin' });
...
export const leadStore = {
  ...
  leads: {
    ...
    delete: async (id) => {
      ...
      // Cascade delete: remove activities for this lead.
      const orphanIds = Array.from(activities.values())
        .filter(a => a.leadId === id)
        .map(a => a.id);
      await Promise.all(orphanIds.map(aid => activities.delete(aid)));
      return true;
    },
