#!/bin/bash
# wallet-twin - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4896}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_wallet_twin_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  wallet-twin - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/analytics/wallet" GET "/api/analytics/wallet" "" "" "200"
