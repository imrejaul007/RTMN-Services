...
function makeTmpDir() { return fs.mkdtempSync(path.join(os.tmpdir(), 'mm-video-an-')); }
...
test('Health & ready', async () => {
  ...
  assert.strictEqual(r.body.service, 'mm-video-analysis');
...
test('Detect scenes: 60s → 6 scenes', async () => {
  const tmp = makeTmpDir(); const port = uniquePort();
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const r = await request(port, 'POST', '/scenes', { data: 'aGVsbG8=', duration_seconds: 60 }, 'tok');
  assert.strictEqual(r.body.result.scene_count, 6);
  assert.strictEqual(r.body.result.scenes[0].index, 0);
  assert.strictEqual(r.body.result.scenes[5].end_seconds, 60);
  await stopService(h); fs.rmSync(tmp, { recursive: true, force: true });
});

test('Detect scenes: 5s → 1 scene (minimum)', async () => {
  const tmp = makeTmpDir(); const port = uniquePort();
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const r = await request(port, 'POST', '/scenes', { data: 'aGVsbG8=', duration_seconds: 5 }, 'tok');
  assert.strictEqual(r.body.result.scene_count, 1);
  await stopService(h); fs.rmSync(tmp, { recursive: true, force: true });
});

test('Classify action', async () => {
  const tmp = makeTmpDir(); const port = uniquePort();
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const r = await request(port, 'POST', '/actions', { data: 'aGVsbG8=' }, 'tok');
  assert.ok(['walking', 'running', 'sitting', 'talking', 'driving', 'cooking', 'reading', 'typing', 'sports', 'dancing'].includes(r.body.result.action));
  await stopService(h); fs.rmSync(tmp, { recursive: true, force: true });
});

test('Detect shots: 30s → 10 shots', async () => {
  const tmp = makeTmpDir(); const port = uniquePort();
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const r = await request(port, 'POST', '/shots', { data: 'aA==', duration_seconds: 30 }, 'tok');
  assert.strictEqual(r.body.result.shot_count, 10);
  assert.strictEqual(r.body.result.shots[0].transition, 'start');
  await stopService(h); fs.rmSync(tmp, { recursive: true, force: true });
});

test('Summarize combines all signals', async () => {
  const tmp = makeTmpDir(); const port = uniquePort();
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const r = await request(port, 'POST', '/summarize', { data: 'aA==', duration_seconds: 60 }, 'tok');
  assert.ok(r.body.result.scenes);
  assert.ok(r.body.result.shots);
  assert.ok(r.body.result.action);
  assert.ok(r.body.result.summary_text);
  await stopService(h); fs.rmSync(tmp, { recursive: true, force: true });
});
...
test('Validation: missing data', async () => {
  const tmp = makeTmpDir(); const port = uniquePort();
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const r = await request(port, 'POST', '/scenes', {}, 'tok');
...
test('Jobs filter by type', async () => {
  ...
  const h = await startService({ PORT: String(port), DATA_DIR: tmp, INTERNAL_TOKEN: 'tok' });
  const tok = 'tok';
  await request(port, 'POST', '/scenes', { data: 'aA==', duration_seconds: 30 }, tok);
  await request(port, 'POST', '/actions', { data: 'aA==' }, tok);
  await request(port, 'POST', '/shots', { data: 'aA==', duration_seconds: 30 }, tok);
  const r = await request(port, 'GET', '/jobs?type=scenes', null, tok);
...
test('Persistence', async () => {
  ...
  const j = await request(port1, 'POST', '/scenes', { data: 'aA==', duration_seconds: 30 }, 'tok');
