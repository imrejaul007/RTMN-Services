/**
 * HOJAI AI - Connector Marketplace
...
test('GET /health returns service info', async () => {
  ...
  assert.strictEqual(b.service, 'connector-marketplace');
  assert.ok(typeof b.counts.listings === 'number');
  assert.ok(typeof b.capabilities === 'object');
});
...
test('GET /api/categories returns seeded categories', async () => {
  ...
  assert.ok(b.categories.some(c => c.id === 'cat-crm'));
});
...
test('GET /api/listings?category= filters', async () => {
  const r = await fetch(`${BASE}/api/listings?category=cat-crm`);
  assert.strictEqual(r.status, 200);
  const b = await r.json();
  b.listings.forEach(l => assert.strictEqual(l.category, 'cat-crm'));
});

test('GET /api/listings?q= filters by search', async () => {
  const r = await fetch(`${BASE}/api/listings?q=salesforce`);
  assert.strictEqual(r.status, 200);
  const b = await r.json();
  assert.ok(b.listings.some(l => l.title.toLowerCase().includes('salesforce')));
});

test('GET /api/listings?featured=true filters', async () => {
  const r = await fetch(`${BASE}/api/listings?featured=true`);
  assert.strictEqual(r.status, 200);
  const b = await r.json();
...
test('POST /api/listings creates a listing', async () => {
  const r = await fetch(`${BASE}/api/listings`, {
    ...
    headers: authHeaders(),
    body: JSON.stringify({
      title: 'Test Connector', price: 29, pricingModel: 'subscription', provider: 'test',
    }),
  });
  assert.strictEqual(r.status, 201);
  const b = await r.json();
  assert.ok(b.id);
  assert.strictEqual(b.title, 'Test Connector');
});
...
test('POST /api/listings 400 when required fields missing', async () => {
  const r = await fetch(`${BASE}/api/listings`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ title: 'No price' }),
  });
  assert.strictEqual(r.status, 400);
});
...
test('DELETE /api/listings/:id removes listing', async () => {
  ...
  const c = await fetch(`${BASE}/api/listings`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ title: 'To Delete', price: 10, pricingModel: 'subscription', provider: 'del' }),
  });
...
test('POST /api/listings/:id/reviews creates a review', async () => {
  ...
  const r = await fetch(`${BASE}/api/listings/${id}/reviews`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ rating: 5, comment: 'Great connector!', reviewer: 'tester' }),
  });
  assert.strictEqual(r.status, 201);
  const b = await r.json();
...
test('POST /api/purchases creates a purchase', async () => {
  ...
  assert.strictEqual(b.buyer, 'test-buyer');
});
...
test('POST /api/installs creates an install record', async () => {
  ...
  assert.strictEqual(r.status, 201);
  const b = await r.json();
  assert.ok(b.instanceId.startsWith('cni-'));
  assert.ok(Array.isArray(b.configSteps));
});

test('POST /api/installs 404 for unknown listing', async () => {
  const r = await fetch(`${BASE}/api/installs`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ listingId: '00000000-0000-0000-0000-000000000000', buyerOrg: 'x' }),
  });
  assert.strictEqual(r.status, 404);
});
...
test('GET /api/installs?buyerOrg= filters', async () => {
  const r = await fetch(`${BASE}/api/installs?buyerOrg=test-org`);
  assert.strictEqual(r.status, 200);
  const b = await r.json();
  b.installs.forEach(i => assert.strictEqual(i.buyerOrg, 'test-org'));
});
...
test('GET /api/audit returns audit entries', async () => {
  ...
  // Should have entries from purchases/installs
...
test('POST /api/listings without auth → 401', async () => {
  const r = await fetch(`${BASE}/api/listings`, {
    ...
    body: JSON.stringify({ title: 'x', price: 1, pricingModel: 'subscription', provider: 'x' }),
  });
