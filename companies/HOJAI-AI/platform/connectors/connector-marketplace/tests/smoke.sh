#!/bin/bash
# connector-marketplace - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4375}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_connector_marketplace_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  connector-marketplace - Smoke"
echo "============================================"
...
run "auto GET /api/listings/featured" GET "/api/listings/featured" "" "" "200"
run "auto GET /api/listings/trending" GET "/api/listings/trending" "" "" "200"
run "auto GET /api/listings/test-id" GET "/api/listings/test-id" "" "" "200"
run "auto POST /api/listings/test-id/reviews" POST "/api/listings/test-id/reviews" '{"test":true}' "" "201"
run "auto POST /api/purchases" POST "/api/purchases" '{"test":true}' "" "201"
run "auto POST /api/installs" POST "/api/installs" '{"test":true}' "" "201"
