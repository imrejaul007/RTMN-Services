#!/bin/bash
# connector-hub - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4216}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_connector_hub_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  connector-hub - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /" GET "/" "" "" "200"
run "auto GET /api/connectors" GET "/api/connectors" "" "" "200"
run "auto GET /api/connectors/test-name" GET "/api/connectors/test-name" "" "" "200"
run "auto GET /api/sync-log" GET "/api/sync-log" "" "" "200"
run "auto POST /api/connections" POST "/api/connections" '{"test":true}' "" "201"
