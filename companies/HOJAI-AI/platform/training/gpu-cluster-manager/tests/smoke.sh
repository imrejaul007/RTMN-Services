#!/bin/bash
# gpu-cluster-manager - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4778}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_gpu_cluster_manager_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  gpu-cluster-manager - Smoke"
echo "============================================"
...
run "auto POST /api/nodes/test-id/heartbeat" POST "/api/nodes/test-id/heartbeat" '{"test":true}' "" "201"
run "auto POST /api/allocate" POST "/api/allocate" '{"test":true}' "" "201"
run "auto GET /api/cluster/stats" GET "/api/cluster/stats" "" "" "200"
run "auto GET /api/gpu-models" GET "/api/gpu-models" "" "" "200"
