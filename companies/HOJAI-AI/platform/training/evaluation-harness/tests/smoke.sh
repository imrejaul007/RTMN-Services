#!/bin/bash
# evaluation-harness - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4027}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_evaluation_harness_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  evaluation-harness - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/benchmarks" POST "/api/benchmarks" '{"test":true}' "" "201"
run "auto GET /api/benchmarks/test-idOrSlug" GET "/api/benchmarks/test-idOrSlug" "" "" "200"
run "auto POST /api/benchmarks/test-idOrSlug/" POST "/api/benchmarks/test-idOrSlug/test-cases" '{"test":true}' "" "201"
run "auto POST /api/run" POST "/api/run" '{"test":true}' "" "201"
run "auto GET /api/runs" GET "/api/runs" "" "" "200"
run "auto POST /api/compare" POST "/api/compare" '{"test":true}' "" "201"
run "auto GET /api/comparisons" GET "/api/comparisons" "" "" "200"
run "auto GET /api/comparisons/test-id" GET "/api/comparisons/test-id" "" "" "200"
run "auto GET /api/scorers" GET "/api/scorers" "" "" "200"
