#!/bin/bash
# Journey Intelligence - Smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4954}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_ji_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  Journey Intelligence - Smoke"
echo "============================================"

run "health"         GET  "/health"    "" "ok"
run "overview"       GET  "/overview"  "" "totalLeads"
run "journey list"   GET  "/journey"          "" ""
run "analytics funnel" GET  "/analytics/funnel"  "" ""
run "analytics overview" GET  "/analytics/overview" "" ""
