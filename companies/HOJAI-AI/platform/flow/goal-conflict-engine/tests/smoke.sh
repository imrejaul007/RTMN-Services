#!/bin/bash
# goal-conflict-engine - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4970}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_goal_conflict_engine_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  goal-conflict-engine - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /" GET "/" "" "" "200"
run "auto GET /api/goals" GET "/api/goals" "" "" "200"
run "auto GET /api/goals/test-id" GET "/api/goals/test-id" "" "" "200"
run "auto POST /api/conflicts/detect" POST "/api/conflicts/detect" '{"test":true}' "" "201"
run "auto POST /api/conflicts/detect-and-stor" POST "/api/conflicts/detect-and-store" '{"test":true}' "" "201"
run "auto GET /api/conflicts" GET "/api/conflicts" "" "" "200"
run "auto GET /api/conflicts/test-id" GET "/api/conflicts/test-id" "" "" "200"
run "auto POST /api/conflicts/test-id/resolve" POST "/api/conflicts/test-id/resolve" '{"test":true}' "" "201"
run "auto GET /api/resolutions" GET "/api/resolutions" "" "" "200"
run "auto GET /api/oppositions" GET "/api/oppositions" "" "" "200"
run "auto POST /api/goals/sync-from-goalos" POST "/api/goals/sync-from-goalos" '{"test":true}' "" "201"
