#!/bin/bash
# simulation-os - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4241}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_simulation_os_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  simulation-os - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/scenarios" POST "/api/scenarios" '{"test":true}' "" "201"
run "auto GET /api/scenarios/test-id" GET "/api/scenarios/test-id" "" "" "200"
run "auto POST /api/scenarios/test-id/rerun" POST "/api/scenarios/test-id/rerun" '{"test":true}' "" "201"
run "auto POST /api/scenarios/test-id/cancel" POST "/api/scenarios/test-id/cancel" '{"test":true}' "" "201"
run "auto POST /api/scenarios/compare" POST "/api/scenarios/compare" '{"test":true}' "" "201"
run "auto GET /api/templates" GET "/api/templates" "" "" "200"
run "auto GET /api/templates/test-id" GET "/api/templates/test-id" "" "" "200"
run "auto GET /api/presets" GET "/api/presets" "" "" "200"
