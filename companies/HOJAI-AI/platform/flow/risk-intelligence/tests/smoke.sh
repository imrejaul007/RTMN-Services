#!/bin/bash
# risk-intelligence - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4755}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_risk_intelligence_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  risk-intelligence - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto POST /api/fraud/score" POST "/api/fraud/score" '{"test":true}' "" "201"
run "auto POST /api/fraud/score/batch" POST "/api/fraud/score/batch" '{"test":true}' "" "201"
run "auto GET /api/fraud/rules" GET "/api/fraud/rules" "" "" "200"
run "auto POST /api/churn/score" POST "/api/churn/score" '{"test":true}' "" "201"
run "auto POST /api/churn/cohort" POST "/api/churn/cohort" '{"test":true}' "" "201"
run "auto GET /api/churn/feature-importance" GET "/api/churn/feature-importance" "" "" "200"
run "auto POST /api/credit/score" POST "/api/credit/score" '{"test":true}' "" "201"
run "auto POST /api/credit/simulate" POST "/api/credit/simulate" '{"test":true}' "" "201"
run "auto POST /api/risk/composite" POST "/api/risk/composite" '{"test":true}' "" "201"
run "auto GET /api/risk/thresholds" GET "/api/risk/thresholds" "" "" "200"
