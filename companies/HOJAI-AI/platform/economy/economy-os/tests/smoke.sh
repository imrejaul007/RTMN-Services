#!/bin/bash
# agent-economy - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4251}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_agent_economy_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto GET /auth/verify" GET "/auth/verify" "" "" "200"
