...
import {
  ...
  InstallationPlan,
  InstallationStep,
  DepartmentPackManifest,
  IndustryExtensionManifest,
  AIWorkerDefinition,
  InstalledDepartment,
  InstalledExtension,
  InstalledWorker,
...
const DEPARTMENT_PACKS: Record<DepartmentType, DepartmentPackManifest> = {
  finance: {
    id: 'finance',
...
const EXTENSION_PACKS: Record<string, IndustryExtensionManifest> = {
  restaurant: {
    id: 'restaurant',
    industry: 'restaurant',
...
export class Installer {
  ...
  private errors: InstallationError[] = [];
  private startTime: number = 0;

  constructor(companyId: string, baseUrl: string = 'http://localhost') {
   ...
   */
  async execute(plan: InstallationPlan): Promise<InstallationResult> {
    this.startTime = Date.now();
    this.errors = [];

    const installedDepartments: InstalledDepartment[] = [];
    const installedExtensions: InstalledExtension[] = [];
    const installedWorkers: InstalledWorker[] = [];

    for (const step of plan.steps) {
      try {
        switch (step.type) {
          case 'department':
            const dept = await this.installDepartment(step.component as DepartmentType);
            if (dept) installedDepartments.push(dept);
            break;

          case 'extension':
            const ext = await this.installExtension(step.component);
            if (ext) installedExtensions.push(ext);
            break;

          case 'worker':
            const worker = await this.installWorker(step.component);
            if (worker) installedWorkers.push(worker);
            break;
   ...
   */
  async installDepartment(dept: DepartmentType): Promise<InstalledDepartment | null> {
   ...
   */
  async installExtension(extId: string): Promise<InstalledExtension | null> {
    const pack = EXTENSION_PACKS[extId];
    if (!pack) {
      // Extension not registered, create minimal entry
      return {
        id: extId,
        industry: extId as IndustryType,
        version: '1.0.0',
   ...
   */
  async installWorker(workerId: string): Promise<InstalledWorker | null> {
    ...
    const type = parts[1];
    const department = parts[2] as DepartmentType;
