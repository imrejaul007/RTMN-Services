#!/bin/bash
# memory-confidence - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4152}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_memory_confidence_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  memory-confidence - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /" GET "/" "" "" "200"
run "auto POST /api/facts" POST "/api/facts" '{"test":true}' "" "201"
run "auto GET /api/facts/test-id" GET "/api/facts/test-id" "" "" "200"
run "auto POST /api/facts/test-id/reinforce" POST "/api/facts/test-id/reinforce" '{"test":true}' "" "201"
run "auto POST /api/facts/test-id/contradict" POST "/api/facts/test-id/contradict" '{"test":true}' "" "201"
run "auto POST /api/facts/test-id/recall" POST "/api/facts/test-id/recall" '{"test":true}' "" "201"
run "auto POST /api/sync-from-memoryos" POST "/api/sync-from-memoryos" '{"test":true}' "" "201"
