#!/bin/bash
# micro-intelligence - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4169}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_micro_intelligence_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  micro-intelligence - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto POST /api/breakers" POST "/api/breakers" '{"test":true}' "" "201"
run "auto GET /api/breakers/test-name" GET "/api/breakers/test-name" "" "" "200"
run "auto PATCH /api/breakers/test-name/state" PATCH "/api/breakers/test-name/state" '{"test":true}' "" "200"
run "auto POST /api/breakers/test-name/reset" POST "/api/breakers/test-name/reset" '{"test":true}' "" "201"
run "auto POST /api/fallbacks" POST "/api/fallbacks" '{"test":true}' "" "201"
run "auto GET /api/fallbacks/test-name" GET "/api/fallbacks/test-name" "" "" "200"
run "auto GET /api/status" GET "/api/status" "" "" "200"
