#!/bin/bash
# graph-database - Auto-generated smoke tests
set -u
...
run() {
  ...
  local body_file="${TMPDIR}/_graph_database_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  graph-database - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto POST /api/nodes" POST "/api/nodes" '{"test":true}' "" "201"
run "auto POST /api/nodes/batch" POST "/api/nodes/batch" '{"test":true}' "" "201"
run "auto GET /api/nodes/test-id" GET "/api/nodes/test-id" "" "" "200"
run "auto POST /api/edges" POST "/api/edges" '{"test":true}' "" "201"
run "auto POST /api/edges/batch" POST "/api/edges/batch" '{"test":true}' "" "201"
run "auto GET /api/edges/test-id" GET "/api/edges/test-id" "" "" "200"
run "auto POST /api/match" POST "/api/match" '{"test":true}' "" "201"
run "auto POST /api/traverse" POST "/api/traverse" '{"test":true}' "" "201"
run "auto POST /api/shortest-path" POST "/api/shortest-path" '{"test":true}' "" "201"
run "auto POST /api/components" POST "/api/components" '{"test":true}' "" "201"
