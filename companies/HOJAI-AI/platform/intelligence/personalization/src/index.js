...
app.get('/api/profiles', (req, res) => {
  ...
  if (req.query.userId) {
    const p = profiles.find(p => p.userId === req.query.userId);
...
app.post('/api/profiles',  (req, res) => {
  ...
  const profiles = readJson('profiles.json') || [];
  if (profiles.find(p => p.userId === userId)) return res.status(409).json({ error: 'Profile already exists' });
  const profile = { userId, name, preferences, traits, affinityScores: {}, interactionCount: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
...
app.post('/api/preferences/:userId/track',  (req, res) => {
  ...
  const idx = profiles.findIndex(p => p.userId === req.params.userId);
  if (idx < 0) return res.status(404).json({ error: 'Profile not found' });
  const event = { id: uuidv4(), userId: req.params.userId, action, itemId, itemType, value, context, timestamp: new Date().toISOString() };
...
app.get('/api/recommendations/:userId', (req, res) => {
  ...
  const topCategories = Object.entries(profile.affinityScores || {}).sort((a, b) => b[1] - a[1]).map(([cat, score]) => ({ category: cat, score }));
  const recommendations = topCategories.slice(0, parseInt(limit)).map((rec, i) => ({
    itemId: `${rec.category}-rec-${i + 1}`, category: rec.category, score: rec.score,
...
const server = app.listen(PORT, () => { console.log(`Personalization OS running on port ${PORT}`); });
