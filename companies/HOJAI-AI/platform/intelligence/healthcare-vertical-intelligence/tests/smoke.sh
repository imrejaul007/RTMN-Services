#!/bin/bash
# healthcare-vertical-intelligence - Smoke
set -u
BASE_URL="${BASE_URL:-http://localhost:4160}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_healthcare_vertical_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  healthcare-vertical-intelligence - Smoke"
echo "============================================"

run "health"           GET "/health" "" "" "200"
run "templates"        GET "/api/templates" "" "" "200"
run "protocols"        GET "/api/protocols" "" "" "200"
run "protocols filter" GET "/api/protocols?icd10=E11" "" "" "200"
run "protocol create"  POST "/api/protocols" '{"name":"e2e","icd10":"E11","steps":["a"]}' "" "201"
