#!/bin/bash
# vector-db - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4497}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_vector_db_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/collections" POST "/api/collections" '{"test":true}' "" "201"
run "auto GET /api/collections/test-name" GET "/api/collections/test-name" "" "" "200"
run "auto POST /api/collections/test-name/vec" POST "/api/collections/test-name/vectors" '{"test":true}' "" "201"
run "auto POST /api/collections/test-name/vec" POST "/api/collections/test-name/vectors/batch" '{"test":true}' "" "201"
run "auto POST /api/collections/test-name/vec" POST "/api/collections/test-name/vectors/delete-batch" '{"test":true}' "" "201"
run "auto POST /api/collections/test-name/sea" POST "/api/collections/test-name/search" '{"test":true}' "" "201"
run "auto POST /api/collections/test-name/sea" POST "/api/collections/test-name/search-by-text" '{"test":true}' "" "201"
run "auto POST /api/query" POST "/api/query" '{"test":true}' "" "201"
