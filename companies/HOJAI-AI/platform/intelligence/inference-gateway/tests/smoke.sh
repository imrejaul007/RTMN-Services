#!/bin/bash
# inference-gateway - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4559}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_inference_gateway_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  inference-gateway - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto GET /api/models" GET "/api/models" "" "" "200"
run "auto POST /api/route" POST "/api/route" '{"test":true}' "" "201"
run "auto POST /api/complete" POST "/api/complete" '{"test":true}' "" "201"
run "auto GET /api/stats" GET "/api/stats" "" "" "200"
run "auto GET /api/stats/reset" GET "/api/stats/reset" "" "" "200"
