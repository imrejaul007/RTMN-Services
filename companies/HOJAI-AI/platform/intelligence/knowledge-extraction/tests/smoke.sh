#!/bin/bash
# knowledge-extraction - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4784}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_knowledge_extraction_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  knowledge-extraction - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto POST /api/ner/extract" POST "/api/ner/extract" '{"test":true}' "" "201"
run "auto GET /api/ner/types" GET "/api/ner/types" "" "" "200"
run "auto POST /api/link" POST "/api/link" '{"test":true}' "" "201"
run "auto POST /api/kb/entities" POST "/api/kb/entities" '{"test":true}' "" "201"
run "auto DELETE /api/kb/entities/test-id" DELETE "/api/kb/entities/test-id" "" "" "200"
run "auto GET /api/kb/stats" GET "/api/kb/stats" "" "" "200"
run "auto POST /api/facts/extract" POST "/api/facts/extract" '{"test":true}' "" "201"
run "auto POST /api/extract-all" POST "/api/extract-all" '{"test":true}' "" "201"
run "auto GET /api/catalog/tech" GET "/api/catalog/tech" "" "" "200"
run "auto GET /api/catalog/persons" GET "/api/catalog/persons" "" "" "200"
