#!/bin/bash
# GraphQL Federation - Smoke tests
set -u
...
run() {
  ...
  local body_file="${TMPDIR}/_gql_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  GraphQL Federation - Smoke"
echo "============================================"

run "health"         GET  "/health" "" "healthy"
run "graphql hello"  POST "/graphql" '{"query":"{ hello }"}' "RTMN GraphQL Federation"
run "graphql status" POST "/graphql" '{"query":"{ status }"}' "operational"
run "graphql svcs"   POST "/graphql" '{"query":"{ services { id name port } }"}' "Sales OS"
run "graphql schema" GET  "/graphql" "" "GraphQL" # GET may 404, that's fine
run "bad query"      POST "/graphql" '{"query":"{ unknown }"}' "Cannot query field"
