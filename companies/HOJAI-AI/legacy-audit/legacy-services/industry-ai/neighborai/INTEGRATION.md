# Neighborai - Integration Guide

## Overview

This document describes how to integrate with Neighborai.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/neighborai
...
```javascript
// GET request
const data = await fetch('http://localhost:4806/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4806/api/v1/resource', {
  method: 'POST',
  headers,
