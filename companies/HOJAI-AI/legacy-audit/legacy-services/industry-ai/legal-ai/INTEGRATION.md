...
This document describes how to integrate with Legal Ai.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/legal-ai
...
```javascript
// GET request
const data = await fetch('http://localhost:4510/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4510/api/v1/resource', {
  method: 'POST',
  headers,
