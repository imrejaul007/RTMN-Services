# Franchise Twin - Integration Guide

## Overview

This document describes how to integrate with Franchise Twin.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/franchise-twin
...
```javascript
// GET request
const data = await fetch('http://localhost:5430/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:5430/api/v1/resource', {
  method: 'POST',
  headers,
