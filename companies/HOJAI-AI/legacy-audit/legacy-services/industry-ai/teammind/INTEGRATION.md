# Teammind - Integration Guide

## Overview

This document describes how to integrate with Teammind.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/teammind
...
```javascript
// GET request
const data = await fetch('http://localhost:4803/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4803/api/v1/resource', {
  method: 'POST',
  headers,
