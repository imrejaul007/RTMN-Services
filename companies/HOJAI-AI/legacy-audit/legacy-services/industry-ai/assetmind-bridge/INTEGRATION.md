# Assetmind Bridge - Integration Guide

## Overview

This document describes how to integrate with Assetmind Bridge.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/assetmind-bridge
...
```javascript
// GET request
const data = await fetch('http://localhost:5310/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:5310/api/v1/resource', {
  method: 'POST',
  headers,
