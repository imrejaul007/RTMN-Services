# Hojai Prospect Context Service - Integration Guide

## Overview

This document describes how to integrate with Hojai Prospect Context Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-prospect-context-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4550/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4550/api/v1/resource', {
  method: 'POST',
  headers,
