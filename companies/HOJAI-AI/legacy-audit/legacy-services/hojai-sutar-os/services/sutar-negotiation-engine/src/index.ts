...
const app = express();
const PORT = process.env.PORT ? parseInt(process.env.PORT) : 4191;
...
app.get("/health", (_req, res) => {
  res.json({ status: "healthy", service: "sutar-negotiation-engine", version: "1.0.0", timestamp: new Date().toISOString(), uptime: Math.floor((Date.now() - START_TIME) / 1000) });
});

const apiResponse = <T>(success: boolean, data?: T, error?: string) => ({ success, data, error, timestamp: new Date().toISOString() });

app.get("/api/v1/info", (_req, res) => {
  res.json(apiResponse(true, { name: "sutar-negotiation-engine", description: "Negotiation Engine", version: "1.0.0", features: ['RFQ processing', 'Counter-offers'] }));
});
...
app.listen(PORT, () => {
  console.log(`SUTAR-NEGOTIATION-ENGINE running on port 4191`);
});
