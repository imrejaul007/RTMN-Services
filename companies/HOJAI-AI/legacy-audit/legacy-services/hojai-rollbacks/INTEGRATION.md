# Hojai Rollbacks - Integration Guide

## Overview

This document describes how to integrate with Hojai Rollbacks.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-rollbacks
...
```javascript
// GET request
const data = await fetch('http://localhost:4594/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4594/api/v1/resource', {
  method: 'POST',
  headers,
