# Hojai Enterprise Search - Integration Guide

## Overview

This document describes how to integrate with Hojai Enterprise Search.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-enterprise-search
...
```javascript
// GET request
const data = await fetch('http://localhost:4620/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4620/api/v1/resource', {
  method: 'POST',
  headers,
