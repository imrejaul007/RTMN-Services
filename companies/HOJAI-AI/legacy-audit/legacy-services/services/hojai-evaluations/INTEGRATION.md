# Hojai Evaluations - Integration Guide

## Overview

This document describes how to integrate with Hojai Evaluations.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-evaluations
...
```javascript
// GET request
const data = await fetch('http://localhost:4591/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4591/api/v1/resource', {
  method: 'POST',
  headers,
