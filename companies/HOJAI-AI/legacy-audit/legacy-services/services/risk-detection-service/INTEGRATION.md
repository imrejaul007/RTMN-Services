# Risk Detection Service - Integration Guide

## Overview

This document describes how to integrate with Risk Detection Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/risk-detection-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4604/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4604/api/v1/resource', {
  method: 'POST',
  headers,
