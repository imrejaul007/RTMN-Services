# Hojai Visual Workflow - Integration Guide

## Overview

This document describes how to integrate with Hojai Visual Workflow.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-visual-workflow
...
```javascript
// GET request
const data = await fetch('http://localhost:4600/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4600/api/v1/resource', {
  method: 'POST',
  headers,
