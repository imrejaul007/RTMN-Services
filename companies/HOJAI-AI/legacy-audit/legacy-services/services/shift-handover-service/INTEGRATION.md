# Shift Handover Service - Integration Guide

## Overview

This document describes how to integrate with Shift Handover Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/shift-handover-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4603/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4603/api/v1/resource', {
  method: 'POST',
  headers,
