# Genie Wake Word Service - Integration Guide

## Overview

This document describes how to integrate with Genie Wake Word Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/genie-wake-word-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4580/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4580/api/v1/resource', {
  method: 'POST',
  headers,
