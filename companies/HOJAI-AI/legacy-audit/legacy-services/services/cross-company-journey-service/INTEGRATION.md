# Cross Company Journey Service - Integration Guide

## Overview

This document describes how to integrate with Cross Company Journey Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/cross-company-journey-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4598/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4598/api/v1/resource', {
  method: 'POST',
  headers,
