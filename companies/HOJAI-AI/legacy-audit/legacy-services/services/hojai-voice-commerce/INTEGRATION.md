# Hojai Voice Commerce - Integration Guide

## Overview

This document describes how to integrate with Hojai Voice Commerce.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-voice-commerce
...
```javascript
// GET request
const data = await fetch('http://localhost:4880/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4880/api/v1/resource', {
  method: 'POST',
  headers,
