# Next Step Intelligence Service - Integration Guide

## Overview

This document describes how to integrate with Next Step Intelligence Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/next-step-intelligence-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4597/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4597/api/v1/resource', {
  method: 'POST',
  headers,
