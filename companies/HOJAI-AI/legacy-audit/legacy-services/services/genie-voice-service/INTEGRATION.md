# Genie Voice Service - Integration Guide

## Overview

This document describes how to integrate with Genie Voice Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/genie-voice-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4712/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4712/api/v1/resource', {
  method: 'POST',
  headers,
