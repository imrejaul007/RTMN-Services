# Leverge Copilot - Integration Guide

## Overview

This document describes how to integrate with Leverge Copilot.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/leverge-copilot
...
```javascript
// GET request
const data = await fetch('http://localhost:4755/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4755/api/v1/resource', {
  method: 'POST',
  headers,
