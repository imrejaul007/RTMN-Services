# Genie Sync Service - Integration Guide

## Overview

This document describes how to integrate with Genie Sync Service.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/genie-sync-service
...
```javascript
// GET request
const data = await fetch('http://localhost:4707/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4707/api/v1/resource', {
  method: 'POST',
  headers,
