/**
 * Logger Utility
 * Simple structured logging for the service
 */
...
class Logger {
  ...
  debug(action: string, data?: Record<string, unknown>): void {
    if (this.shouldLog('debug')) {
      this.output(this.formatLog('debug', action, data));
    }
  }

  info(action: string, data?: Record<string, unknown>): void {
    if (this.shouldLog('info')) {
      this.output(this.formatLog('info', action, data));
    }
  }

  warn(action: string, data?: Record<string, unknown>): void {
    if (this.shouldLog('warn')) {
      this.output(this.formatLog('warn', action, data));
    }
  }

  error(action: string, data?: Record<string, unknown>): void {
    if (this.shouldLog('error')) {
      this.output(this.formatLog('error', action, data));
    }
  }
}
