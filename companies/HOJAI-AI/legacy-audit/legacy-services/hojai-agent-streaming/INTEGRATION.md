# Hojai Agent Streaming - Integration Guide

## Overview

This document describes how to integrate with Hojai Agent Streaming.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-agent-streaming
...
```javascript
// GET request
const data = await fetch('http://localhost:4601/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4601/api/v1/resource', {
  method: 'POST',
  headers,
