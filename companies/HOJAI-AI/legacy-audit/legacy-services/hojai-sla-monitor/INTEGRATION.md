# Hojai Sla Monitor - Integration Guide

## Overview

This document describes how to integrate with Hojai Sla Monitor.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-sla-monitor
...
```javascript
// GET request
const data = await fetch('http://localhost:4920/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4920/api/v1/resource', {
  method: 'POST',
  headers,
