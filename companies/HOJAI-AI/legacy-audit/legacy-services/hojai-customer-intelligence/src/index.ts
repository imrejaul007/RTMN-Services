/**
 * HOJAI Customer Intelligence - Main Server
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/hojai-customer-intelligence';
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.on('finish', () => logger.info('request_completed', { requestId, method: req.method, path: req.path, status: res.statusCode, durationMs: Date.now() - startTime }));
  res.setHeader('X-Request-ID', requestId);
  next();
});

app.get('/health', (req: Request, res: Response) => {
  res.json({ status: 'healthy', service: 'hojai-customer-intelligence', version: '1.0.0', timestamp: new Date().toISOString() });
});
