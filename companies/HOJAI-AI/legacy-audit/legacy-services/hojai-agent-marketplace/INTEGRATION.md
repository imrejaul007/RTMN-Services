# Hojai Agent Marketplace - Integration Guide

## Overview

This document describes how to integrate with Hojai Agent Marketplace.
...
```bash
export MONGODB_URI=mongodb://localhost:27017/hojai-agent-marketplace
...
```javascript
// GET request
const data = await fetch('http://localhost:4860/api/v1/resource', {
  method: 'GET',
  headers
});

// POST request
const created = await fetch('http://localhost:4860/api/v1/resource', {
  method: 'POST',
  headers,
