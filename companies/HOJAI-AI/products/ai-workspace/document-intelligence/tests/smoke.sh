#!/bin/bash
# document-intelligence - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4782}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_document_intelligence_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  document-intelligence - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/health" GET "/api/health" "" "" "200"
run "auto POST /api/extract" POST "/api/extract" '{"test":true}' "" "201"
run "auto POST /api/extract/batch" POST "/api/extract/batch" '{"test":true}' "" "201"
run "auto POST /api/extract-and-rag" POST "/api/extract-and-rag" '{"test":true}' "" "201"
run "auto GET /api/formats" GET "/api/formats" "" "" "200"
