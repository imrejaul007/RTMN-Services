#!/bin/bash
# context-engine - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4746}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_context_engine_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  context-engine - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/contexts" POST "/api/contexts" '{"test":true}' "" "201"
run "auto GET /api/contexts/active" GET "/api/contexts/active" "" "" "200"
run "auto GET /api/contexts/test-id" GET "/api/contexts/test-id" "" "" "200"
run "auto POST /api/contexts/test-id/spans" POST "/api/contexts/test-id/spans" '{"test":true}' "" "201"
run "auto GET /api/contexts/test-id/timeline" GET "/api/contexts/test-id/timeline" "" "" "200"
run "auto POST /api/contexts/test-id/logs" POST "/api/contexts/test-id/logs" '{"test":true}' "" "201"
run "auto POST /api/contexts/lookup" POST "/api/contexts/lookup" '{"test":true}' "" "201"
run "auto POST /api/contexts/propagate" POST "/api/contexts/propagate" '{"test":true}' "" "201"
