#!/bin/bash
# whatsapp-os - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4860}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_whatsapp_os_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/messages/send" POST "/api/messages/send" '{"test":true}' "" "201"
run "auto POST /api/webhook/simulate" POST "/api/webhook/simulate" '{"test":true}' "" "201"
run "auto GET /api/conversations" GET "/api/conversations" "" "" "200"
run "auto GET /api/contacts" GET "/api/contacts" "" "" "200"
run "auto GET /api/messages" GET "/api/messages" "" "" "200"
