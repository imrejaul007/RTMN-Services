#!/bin/bash
# email-os - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4862}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_email_os_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
run "auto POST /api/templates/test-id/render" POST "/api/templates/test-id/render" '{"test":true}' "" "201"
run "auto POST /api/emails/send" POST "/api/emails/send" '{"test":true}' "" "201"
run "auto POST /api/inbox/receive" POST "/api/inbox/receive" '{"test":true}' "" "201"
run "auto GET /api/inbox" GET "/api/inbox" "" "" "200"
run "auto GET /api/inbox/test-id" GET "/api/inbox/test-id" "" "" "200"
run "auto POST /api/inbox/test-id/draft-reply" POST "/api/inbox/test-id/draft-reply" '{"test":true}' "" "201"
run "auto GET /api/threads" GET "/api/threads" "" "" "200"
run "auto GET /api/threads/test-id" GET "/api/threads/test-id" "" "" "200"
