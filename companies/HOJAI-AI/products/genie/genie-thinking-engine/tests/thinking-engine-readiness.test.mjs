/**
 * genie-thinking-engine — readiness + seed smoke tests
 *
 * Verifies:
 *   - /health returns 200 with thinking-engine capabilities
 ...
 *   - /ready still works
 *   - Seed data populated PersistentMap stores
 *   - Service actually starts and is reachable
 */
...
const INTERNAL_TOKEN = 'th-test-token-' + Date.now();
...
async function bearer() {
  // createToken in ESM auth/index.js requires JWT_SECRET (HS256 signing)
  const tok = await createToken({ userId: 'TH-USER-1', businessId: 'TH-BIZ', industry: 'test', role: 'owner' });
  return {
...
async function getBearer() {
  if (!bearerHeaders) bearerHeaders = await bearer();
...
function spawnService(srcPath, env, label) {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        const dataDir = mkdtempSync(path.join(tmpdir(), `th-${label}-data-`));
        const child = spawn('node', [srcPath], {
          cwd: mkdtempSync(path.join(tmpdir(), `th-${label}-cwd-`)),
          env: {
            ...process.env,
            ...env,
            PORT: String(port),
            INTERNAL_SERVICE_TOKEN: INTERNAL_TOKEN,
            JWT_SECRET: 'th-test-jwt-secret-' + Date.now(),
...
async function run() {
  console.log('\n[genie-thinking-engine] readiness tests:\n');

  const { child, port } = await spawnService(SVC_SRC, {}, 'th');
  ...
  a('/health identifies thinking-engine', health.data?.service === 'genie-thinking-engine');
  ...
  a('/api/llm-health has success flag', llm.data?.success === true);
  a('/api/llm-health identifies thinking-engine', llm.data?.data?.service === 'genie-thinking-engine');
  ...
  a('/api/db-health identifies thinking-engine', db.data?.data?.service === 'genie-thinking-engine');
  ...
  console.log(`\ngenie-thinking-engine readiness: ${p} passed, ${f} failed`);
