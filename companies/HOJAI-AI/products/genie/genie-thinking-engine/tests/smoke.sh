#!/bin/bash
# genie-thinking-engine - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4719}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_thinking_engine_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-thinking-engine - Smoke"
echo "============================================"
