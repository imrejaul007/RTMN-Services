#!/bin/bash
# Genie Life GPS - Smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4721}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_glg_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  Genie Life GPS - Smoke"
echo "============================================"

run "health"             GET  "/health"                  "" "healthy"
run "ready"              GET  "/ready"                   "" ""
run "gps next u1"        GET  "/gps/next/u1"             "" ""
run "gps where u1"       GET  "/gps/where/u1"            "" ""
run "future self u1"     GET  "/future/self/u1"          "" ""
run "future scenarios"   GET  "/future/scenarios/u1"     "" ""
run "vision u1"          GET  "/goals/vision/u1"         "" ""
run "create life goal"   POST "/goals/life"              '{"userId":"u1","title":"Goal","category":"career"}' ""
