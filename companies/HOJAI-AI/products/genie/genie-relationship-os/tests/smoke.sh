#!/bin/bash
# genie-relationship-os - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4718}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_relationship_os_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-relationship-os - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/test-user/dashboard" GET "/api/test-user/dashboard" "" "" "200"
