/**
 * genie-listening-modes — Phase A readiness tests.
 *
 * Verifies:
 *   - /health returns 200
 *   - /api/llm-health, /api/db-health, /api/readiness all return 200 with expected fields
 *   - /ready returns 200
 *   - /api/modes returns 4 listening modes
 *   - /api/stats returns mode usage stats
 *   - /api/modes/webhook-log returns seeded webhook-log entries (Phase A augmentation)
 */
...
const INTERNAL_TOKEN = 'lm-readiness-test-token-' + Date.now();
...
function spawnService() {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        const dataDir = mkdtempSync(path.join(tmpdir(), 'lm-readiness-data-'));
        const cwd = mkdtempSync(path.join(tmpdir(), 'lm-readiness-cwd-'));
...
async function run() {
  console.log('\n[genie-listening-modes readiness] tests:\n');
  ...
  a('/health service=genie-listening-modes', h.data?.service === 'genie-listening-modes');
  a('/health counts.modes=4', h.data?.counts?.modes === 4);
  a('/health counts.devices=5 (existing seed)', h.data?.counts?.devices === 5);
  ...
  a('/api/llm-health service=genie-listening-modes', lh.data?.data?.service === 'genie-listening-modes');
  ...
  a('/api/db-health service=genie-listening-modes', dh.data?.data?.service === 'genie-listening-modes');
  ...
  // === /api/modes ===
  const modes = await req('GET', svc.port, '/api/modes');
  a('/api/modes returns 200', modes.status === 200);
  a('/api/modes has 4 modes', modes.data?.total === 4);
  a('/api/modes includes manual', !!modes.data?.modes?.manual);
  a('/api/modes includes continuous', !!modes.data?.modes?.continuous);

  // === /api/stats ===
  const stats = await req('GET', svc.port, '/api/stats');
  a('/api/stats returns 200', stats.status === 200);
  a('/api/stats has modeUsage field', typeof stats.data?.modeUsage === 'object');

  // === /api/webhook-log (Phase A augmented) ===
  const wl = await req('GET', svc.port, '/api/webhook-log');
  a('/api/webhook-log returns 200', wl.status === 200);
  a('/api/webhook-log has 5 entries', wl.data?.total === 5);
  a('/api/webhook-log wh1 action=start', wl.data?.webhookLog?.find(w => w.id === 'wh1')?.action === 'start');

  await killChild(svc.child);

  console.log(`\ngenie-listening-modes readiness: ${p} passed, ${f} failed`);
