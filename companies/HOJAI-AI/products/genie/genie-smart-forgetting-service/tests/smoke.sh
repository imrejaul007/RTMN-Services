#!/bin/bash
# genie-smart-forgetting-service - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4715}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_smart_forgetting_service_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-smart-forgetting-service - Smoke"
echo "============================================"
