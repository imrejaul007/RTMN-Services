#!/bin/bash
# Genie Money OS - Smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4724}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_gmo_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  Genie Money OS - Smoke"
echo "============================================"

run "health"           GET  "/health"              "" "healthy"
run "service info"     GET  "/"                    "" "Genie Money OS"
run "expenses u1"      GET  "/expenses/u1"         "" ""
run "expense categories" GET "/expenses/categories/all" "" ""
run "budget categories"  GET "/budget/defaults/categories" "" ""
run "savings insights"   GET "/insights/u1/savings-insights" "" ""
run "goals u1"         GET  "/goals/u1"            "" ""
