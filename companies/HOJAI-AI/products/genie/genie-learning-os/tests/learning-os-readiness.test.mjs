/**
 * genie-learning-os — readiness + seed smoke tests
 *
 * Verifies:
 *   - /health returns 200
 ...
 *   - /ready still works
 *   - Representative route GET (e.g., /curriculum) returns data
 */
...
const INTERNAL_TOKEN = 'ln-test-token-' + Date.now();
...
async function getBearer() {
  const tok = await createToken({ userId: 'LN-USER-1', businessId: 'LN-BIZ', industry: 'test', role: 'owner' });
  return {
...
function spawnService(srcPath, env, label) {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        const dataDir = mkdtempSync(path.join(tmpdir(), `ln-${label}-data-`));
        const child = spawn('node', [srcPath], {
          cwd: mkdtempSync(path.join(tmpdir(), `ln-${label}-cwd-`)),
          env: {
            ...process.env,
            ...env,
            PORT: String(port),
            INTERNAL_SERVICE_TOKEN: INTERNAL_TOKEN,
            JWT_SECRET: 'ln-test-jwt-secret-' + Date.now(),
...
async function run() {
  console.log('\n[genie-learning-os] readiness tests:\n');

  const { child, port } = await spawnService(SVC_SRC, {}, 'ln');
  ...
  a('/health identifies learning-os', health.data?.service === 'genie-learning-os');
  ...
  a('/api/llm-health identifies learning-os', llm.data?.data?.service === 'genie-learning-os');
  ...
  a('/api/db-health identifies learning-os', db.data?.data?.service === 'genie-learning-os');
  ...
  // === Representative route GET (curriculum listing) ===
  const curriculum = await req('GET', port, '/curriculum');
  if (curriculum.status !== 200) console.log('    (debug /curriculum: status=' + curriculum.status + ')');
  a('/curriculum returns 200', curriculum.status === 200);

  await killChild(child);

  console.log(`\ngenie-learning-os readiness: ${p} passed, ${f} failed`);
