#!/bin/bash
# genie-companion-service - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4716}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_companion_service_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-companion-service - Smoke"
echo "============================================"
