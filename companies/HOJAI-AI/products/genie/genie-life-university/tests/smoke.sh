#!/bin/bash
# Genie Life University - Smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4727}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_glu_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  Genie Life University - Smoke"
echo "============================================"

run "health"               GET  "/health"                  "" "healthy"
run "service info"         GET  "/"                        "" "Genie Life University Engine"
run "courses list"         GET  "/courses"                 "" ""
run "curriculum paths"     GET  "/curriculum/paths"        "" ""
run "curriculum u1"        GET  "/curriculum/u1"           "" ""
run "cert templates"       GET  "/certification/templates/all" "" ""
run "cert u1"              GET  "/certification/u1"        "" ""
run "recommendations"      POST "/curriculum/recommendations" '{"userId":"u1","interests":["ai"]}' ""
