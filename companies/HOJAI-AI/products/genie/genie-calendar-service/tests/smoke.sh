#!/bin/bash
# genie-calendar-service - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4709}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_calendar_service_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-calendar-service - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /" GET "/" "" "" "200"
run "auto GET /api/types" GET "/api/types" "" "" "200"
run "auto GET /api/events" GET "/api/events" "" "" "200"
run "auto GET /api/events/today" GET "/api/events/today" "" "" "200"
run "auto GET /api/events/upcoming" GET "/api/events/upcoming" "" "" "200"
run "auto GET /api/events/search" GET "/api/events/search" "" "" "200"
run "auto GET /api/events/test-id" GET "/api/events/test-id" "" "" "200"
run "auto GET /api/availability" GET "/api/availability" "" "" "200"
run "auto POST /api/availability/find" POST "/api/availability/find" '{"test":true}' "" "201"
run "auto POST /api/conflicts" POST "/api/conflicts" '{"test":true}' "" "201"
run "auto GET /api/conflicts/test-user" GET "/api/conflicts/test-user" "" "" "200"
