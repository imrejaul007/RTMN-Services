/**
 * genie-creation-os — readiness + seed smoke tests
 *
 * Verifies:
 *   - /health returns 200
 ...
 *   - Representative route GET (e.g., /content/types) returns data
 */
...
const INTERNAL_TOKEN = 'cr-test-token-' + Date.now();
...
async function getBearer() {
  const tok = await createToken({ userId: 'CR-USER-1', businessId: 'CR-BIZ', industry: 'test', role: 'owner' });
  return {
...
function spawnService(srcPath, env, label) {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        const dataDir = mkdtempSync(path.join(tmpdir(), `cr-${label}-data-`));
        const child = spawn('node', [srcPath], {
          cwd: mkdtempSync(path.join(tmpdir(), `cr-${label}-cwd-`)),
          env: {
            ...process.env,
            ...env,
            PORT: String(port),
            INTERNAL_SERVICE_TOKEN: INTERNAL_TOKEN,
            JWT_SECRET: 'cr-test-jwt-secret-' + Date.now(),
...
async function run() {
  console.log('\n[genie-creation-os] readiness tests:\n');

  const { child, port } = await spawnService(SVC_SRC, {}, 'cr');
  ...
  a('/api/llm-health identifies creation-os', llm.data?.data?.service === 'genie-creation-os');
  ...
  a('/api/db-health identifies creation-os', db.data?.data?.service === 'genie-creation-os');
  ...
  a('root listing names creation-os', root.data?.service === 'Genie Creation OS');
  a('root listing exposes capabilities array', Array.isArray(root.data?.capabilities));

  // === Representative route GET (content types) ===
  const types = await req('GET', port, '/content/types');
  if (types.status !== 200) console.log('    (debug /content/types: status=' + types.status + ')');
  a('/content/types returns 200', types.status === 200);

  await killChild(child);

  console.log(`\ngenie-creation-os readiness: ${p} passed, ${f} failed`);
