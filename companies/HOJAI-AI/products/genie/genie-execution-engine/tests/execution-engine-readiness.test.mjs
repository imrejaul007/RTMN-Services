/**
 * genie-execution-engine — readiness + seed smoke tests
 *
 * Verifies:
 *   - /health returns 200
 ...
 *   - /ready still works
 *   - GET / (root listing) returns capabilities
 *   - Representative route GET (e.g., /tasks/priorities/all) returns data
 */
...
const INTERNAL_TOKEN = 'ex-test-token-' + Date.now();
...
async function getBearer() {
  const tok = await createToken({ userId: 'EX-USER-1', businessId: 'EX-BIZ', industry: 'test', role: 'owner' });
  return {
...
function spawnService(srcPath, env, label) {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        const dataDir = mkdtempSync(path.join(tmpdir(), `ex-${label}-data-`));
        const child = spawn('node', [srcPath], {
          cwd: mkdtempSync(path.join(tmpdir(), `ex-${label}-cwd-`)),
          env: {
            ...process.env,
            ...env,
            PORT: String(port),
            INTERNAL_SERVICE_TOKEN: INTERNAL_TOKEN,
            JWT_SECRET: 'ex-test-jwt-secret-' + Date.now(),
        ...
        let resolved = false;
        const onLine = (line) => {
          if (line.includes('running on port') && !resolved) {
            resolved = true;
        ...
        setTimeout(() => {
          if (!resolved) {
            resolved = true;
            child.kill('SIGTERM');
            reject(new Error(`Service ${label} didn't print running on port within 10s:\n${stdout}\n${stderr}`));
          }
        }, 10000);
      });
    });
...
async function run() {
  console.log('\n[genie-execution-engine] readiness tests:\n');

  const { child, port } = await spawnService(SVC_SRC, {}, 'ex');
  ...
  a('/health reports healthy status', health.data?.status === 'healthy');
  ...
  a('/api/llm-health identifies execution-engine', llm.data?.data?.service === 'genie-execution-engine');
  ...
  a('/api/db-health identifies execution-engine', db.data?.data?.service === 'genie-execution-engine');
  ...
  // === Root listing ===
  const root = await req('GET', port, '/');
  a('root listing returns 200', root.status === 200);
  a('root listing names execution engine', root.data?.service === 'Genie Execution Engine');
  a('root listing exposes capabilities array', Array.isArray(root.data?.capabilities));

  // === Representative route GET (task priorities) ===
  const priorities = await req('GET', port, '/tasks/priorities/all');
  if (priorities.status !== 200) console.log('    (debug /tasks/priorities/all: status=' + priorities.status + ')');
  a('/tasks/priorities/all returns 200', priorities.status === 200);

  await killChild(child);

  console.log(`\ngenie-execution-engine readiness: ${p} passed, ${f} failed`);
