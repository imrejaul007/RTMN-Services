#!/bin/bash
# genie-gateway - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4701}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_gateway_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-gateway - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/user/test-user/context" GET "/api/user/test-user/context" "" "" "200"
run "auto GET /api/user/test-user/preference" GET "/api/user/test-user/preferences" "" "" "200"
run "auto POST /api/query" POST "/api/query" '{"test":true}' "" "201"
run "auto GET /api/twins/test-user" GET "/api/twins/test-user" "" "" "200"
run "auto GET /api/briefing/test-user" GET "/api/briefing/test-user" "" "" "200"
run "auto POST /api/memory" POST "/api/memory" '{"test":true}' "" "201"
run "auto GET /api/memory/test-user" GET "/api/memory/test-user" "" "" "200"
run "auto GET /api/search" GET "/api/search" "" "" "200"
run "auto POST /api/broadcast" POST "/api/broadcast" '{"test":true}' "" "201"
