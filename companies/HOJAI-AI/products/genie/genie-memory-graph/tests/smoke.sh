#!/bin/bash
# genie-memory-graph - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4717}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_genie_memory_graph_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  genie-memory-graph - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/user/test-user/graph" GET "/api/user/test-user/graph" "" "" "200"
