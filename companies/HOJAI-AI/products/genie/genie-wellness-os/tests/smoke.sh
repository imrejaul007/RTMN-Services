#!/bin/bash
# Genie Wellness OS - Smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4723}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_gwo_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  Genie Wellness OS - Smoke"
echo "============================================"

run "health"             GET  "/health"               "" "healthy"
run "service info"       GET  "/"                     "" "Genie Wellness OS"
run "health profile u1"  GET  "/health/profile/u1"    "" ""
run "daily u1"           GET  "/health/daily/u1"      "" ""
run "fitness dashboard"  GET  "/fitness/dashboard/u1" "" ""
run "sleep profile"      GET  "/sleep/profile/u1"     "" ""
run "sleep factors"      GET  "/sleep/factors"        "" ""
