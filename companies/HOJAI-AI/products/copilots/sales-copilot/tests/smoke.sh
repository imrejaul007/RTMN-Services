#!/bin/bash
# sales-copilot - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4435}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sales_copilot_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sales-copilot - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/prioritize" POST "/api/prioritize" '{"test":true}' "" "201"
run "auto POST /api/talking-points" POST "/api/talking-points" '{"test":true}' "" "201"
run "auto POST /api/recommend" POST "/api/recommend" '{"test":true}' "" "201"
run "auto POST /api/email/generate" POST "/api/email/generate" '{"test":true}' "" "201"
run "auto GET /api/competitors" GET "/api/competitors" "" "" "200"
run "auto GET /api/competitors/test-id" GET "/api/competitors/test-id" "" "" "200"
run "auto POST /api/meeting/prepare" POST "/api/meeting/prepare" '{"test":true}' "" "201"
