#!/bin/bash
# agent-copilot - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4920}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_agent_copilot_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  agent-copilot - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/agents" GET "/api/agents" "" "" "200"
run "auto GET /api/agents/test-id" GET "/api/agents/test-id" "" "" "200"
run "auto POST /api/execute" POST "/api/execute" '{"test":true}' "" "201"
run "auto GET /api/execute/test-id" GET "/api/execute/test-id" "" "" "200"
run "auto GET /api/executions" GET "/api/executions" "" "" "200"
run "auto POST /api/execute/test-id/cancel" POST "/api/execute/test-id/cancel" '{"test":true}' "" "201"
run "auto POST /api/workflows" POST "/api/workflows" '{"test":true}' "" "201"
run "auto GET /api/workflows/test-id" GET "/api/workflows/test-id" "" "" "200"
run "auto POST /api/workflows/test-id/execute" POST "/api/workflows/test-id/execute" '{"test":true}' "" "201"
run "auto GET /api/skills" GET "/api/skills" "" "" "200"
run "auto GET /api/skills/test-id" GET "/api/skills/test-id" "" "" "200"
