#!/bin/bash
# finance-copilot - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4610}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_finance_copilot_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  finance-copilot - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/cashflow/forecast" POST "/api/cashflow/forecast" '{"test":true}' "" "201"
run "auto POST /api/budget/analyze" POST "/api/budget/analyze" '{"test":true}' "" "201"
run "auto POST /api/anomaly/detect" POST "/api/anomaly/detect" '{"test":true}' "" "201"
run "auto POST /api/refund/analyze" POST "/api/refund/analyze" '{"test":true}' "" "201"
run "auto GET /api/kpis" GET "/api/kpis" "" "" "200"
run "auto GET /api/kpis/test-id" GET "/api/kpis/test-id" "" "" "200"
run "auto POST /api/alerts" POST "/api/alerts" '{"test":true}' "" "201"
run "auto GET /api/insights" GET "/api/insights" "" "" "200"
run "auto POST /api/fraud/detect" POST "/api/fraud/detect" '{"test":true}' "" "201"
