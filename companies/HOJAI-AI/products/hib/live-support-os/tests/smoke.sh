#!/bin/bash
# live-support-os - Auto-generated smoke tests
set -u
...
run() {
  ...
  local body_file="${TMPDIR}/_live_support_os_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  live-support-os - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/queues" GET "/api/queues" "" "" "200"
run "auto GET /api/agents" GET "/api/agents" "" "" "200"
run "auto GET /api/agents/available" GET "/api/agents/available" "" "" "200"
run "auto PATCH /api/agents/test-id/status" PATCH "/api/agents/test-id/status" '{"test":true}' "" "200"
...
run "auto POST /api/sessions/test-id/close" POST "/api/sessions/test-id/close" '{"test":true}' "" "201"
run "auto GET /api/handoffs/test-id" GET "/api/handoffs/test-id" "" "" "200"
