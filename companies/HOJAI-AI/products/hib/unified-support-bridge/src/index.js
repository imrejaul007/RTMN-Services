...
function rateLimit(req, res, next) {
  ...
  if (now - entry.windowStart > RATE_LIMIT_WINDOW_MS) { entry = { count: 0, windowStart: now }; }
  entry.count++;
...
function webhookApiKeyAuth(req, res, next) {
  ...
  if (!key || !WEBHOOK_API_KEYS[key]) { return res.status(401).json({ error: 'unauthorized' }); }
  next();
}
...
// Normalize email string to lowercase trimmed
...
// Extract email address from email object {email, name} or plain string
function getEmailAddress(emailObj) {
  if (!emailObj) return null;
  if (typeof emailObj === 'string') return normalizeEmailStr(emailObj);
  if (emailObj.email) return normalizeEmailStr(emailObj.email);
  return null;
}

function getEmailName(emailObj) {
  if (!emailObj) return null;
  if (typeof emailObj === 'object') return emailObj.name || null;
  return null;
}
...
async function resolveCorpIdCustomer(email) {
  var nEmail = normalizeEmailStr(email);
...
async function resolveCustomerId(opts) {
  opts = opts || {};
  var nPhone = normalizePhone(opts.phone);
  var nEmail = normalizeEmailStr(opts.email);
  var appUserId = opts.appUserId;
  var name = opts.name;
  var metadata = opts.metadata || {};

  // Fast path: direct channel lookup
  if (nPhone) { var ex = await storage.findCustomerByChannel('phone', nPhone); if (ex) return ex; }
  if (nEmail) { var ex = await storage.findCustomerByChannel('email', nEmail); if (ex) return ex; }
  if (appUserId) { var ex = await storage.findCustomerByChannel('app', appUserId); if (ex) return ex; }

  // Cross-channel: appUserId + phone/email links to existing
  ...
  if (nEmail) {
    ...
    if (corpId) {
      ...
      var channels = ['email'];
      if (nPhone) channels.push('phone');
      if (appUserId) channels.push('app');
      await storage.upsertCustomer({ customerId: corpId, phone: nPhone, email: nEmail, appUserId: appUserId, name: name || null, channels: channels, metadata: metadata, linkedFromCorpId: true });
...
app.get('/health', async function(req, res) {
  ...
  try {
    ...
    var r1 = await fetch(UNIFIED_INBOX_URL + '/health', { signal: c1.signal }); clearTimeout(t1);
  ...
  try {
    ...
    var r2 = await fetch(TICKET_ENGINE_URL + '/health', { signal: c2.signal }); clearTimeout(t2);
  ...
  try {
    ...
    var r3 = await fetch(CORPID_URL + '/health', { signal: c3.signal }); clearTimeout(t3);
...
app.post('/api/admin/webhooks/whatsapp/register', rateLimit, async function(req, res) {
  ...
  var result = null;
  if (provider === 'meta') {
    result = await whatsappWebhook.registerMetaWebhook(req.body.accessToken, req.body.phoneNumberId, webhookUrl, WHATSAPP_VERIFY_TOKEN);
  } else if (provider === '360dialog') {
    result = await whatsappWebhook.register360dialogWebhook(req.body.apiKey, webhookUrl, WHATSAPP_VERIFY_TOKEN);
...
app.get('/api/admin/webhooks/whatsapp/status', async function(req, res) {
  ...
  try {
    ...
    var r = await fetch(webhookUrl + '?hub.mode=subscribe&hub.verify_token=test&hub.challenge=ok', { signal: c.signal }); clearTimeout(t);
    var body = await r.text();
    res.json({ url: webhookUrl, reachable: r.status === 200 && body === 'ok', statusCode: r.status, publicUrlConfigured: !!process.env.WEBHOOK_PUBLIC_URL });
  } catch(e) {
...
// Email Webhook
// email.from = { email: string, name: string|null } from emailHandler.normalizeEmail
app.post('/api/webhooks/email', webhookApiKeyAuth, rateLimit, async function(req, res) {
  var email = await emailHandler.normalizeEmail(req.body, req.headers);
  if (!email) return res.status(400).json({ error: 'Invalid email format' });

  var nFrom = getEmailAddress(email.from);
  if (!nFrom) return res.status(400).json({ error: 'from_email_required' });

  var customerId = await resolveCustomerId({
    email: nFrom,
    name: getEmailName(email.from),
...
app.post('/api/webhooks/app', webhookApiKeyAuth, rateLimit, async function(req, res) {
  ...
  if (body.message) {
    await addMessage({ conversationId: result.conversationId, content: body.message, sender: 'customer', channel: 'chat', direction: 'inbound', metadata: { customerName: body.contactName || body.customerName, sessionId: body.sessionId, platform: body.platform } });
...
app.post('/api/customers/link', async function(req, res) {
  ...
  if (body.email) await storage.registerChannelLink(customerId, 'email', normalizeEmailStr(body.email));
...
app.get('/api/customers/:id/activity', async function(req, res) {
  ...
  res.json({ success: true, customerId: req.params.id, totalMessages: messages.length, activity: messages.map(function(m) {
...
app.post('/api/conversations/:id/merge', async function(req, res) {
  ...
  for (var j = 0; j < toMerge.length; j++) {
    var c = await storage.getConversation(toMerge[j]);
    if (c && c.channel) allChannels[c.channel] = true;
  }

  await storage.updateConversation(primary, { linkedConversations: toMerge.filter(function(id) { return id !== primary; }), mergedAt: new Date().toISOString() });
  for (var k = 1; k < toMerge.length; k++) {
    await storage.updateConversation(toMerge[k], { mergedInto: primary, status: 'merged', mergedAt: new Date().toISOString() });
  }

  eventsModule.emitConversationMerged({ primaryConversationId: primary, mergedFrom: toMerge.filter(function(id) { return id !== primary; }), customerId: customerId, channels: Object.keys(allChannels) });

  var customer = await storage.getCustomer(customerId);
  res.json({ success: true, primaryConversationId: primary, mergedFrom: toMerge.filter(function(id) { return id !== primary; }), customerId: customerId, channels: Object.keys(allChannels), customer: { name: customer ? customer.name : null, email: customer ? customer.email : null, phone: customer ? customer.phone : null } });
});
...
app.get('/api/stats', async function(req, res) {
  ...
  res.json({ success: true, stats: { totalCustomers: customers.length, totalConversations: conversations.length, byChannel: byChannel, multiChannelCustomers: customers.filter(function(c) { return (c.channels || []).length > 1; }).length }});
});
...
if (SMTP_PORT > 0) {
  var smtp = emailHandler.createSmtpReceiver(async function(email) {
    var nFrom = getEmailAddress(email.from);
    if (!nFrom) return;
    var customerId = await resolveCustomerId({ email: nFrom, name: getEmailName(email.from), metadata: { source: 'email', messageId: email.messageId, provider: 'smtp' } });
    var convs = await storage.getConversationsByCustomer(customerId);
    var conversationId = null;
    if (email.inReplyTo) {
      for (var i = 0; i < convs.length; i++) {
    ...
    console.log('[smtp] from ' + nFrom + ' -> conv ' + conversationId);
  }, SMTP_PORT);
...
if (process.env.IMAP_USER && process.env.IMAP_PASSWORD) {
  var imap = emailHandler.createImapPoller(async function(email) {
    var nFrom = getEmailAddress(email.from);
    if (!nFrom) return;
    var customerId = await resolveCustomerId({ email: nFrom, name: getEmailName(email.from), metadata: { source: 'email', messageId: email.messageId, provider: 'imap' } });
    var convs = await storage.getConversationsByCustomer(customerId);
    var conversationId = null;
    if (email.inReplyTo) {
      for (var i = 0; i < convs.length; i++) {
...
if (require.main === module) {
  ...
  try { eventsModule.initWebSocket(server); } catch(e) { console.log('[ws] disabled: ' + e.message); }
  if (process.env.REDIS_URL) {
    eventsModule.initRedisPubSub(process.env.REDIS_URL).catch(function(e) { console.warn('[events] Redis pub/sub: ' + e.message); });
  }
