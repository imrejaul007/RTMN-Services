#!/bin/bash
# Pilot Onboarding - Smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4963}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_pon_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  Pilot Onboarding - Smoke"
echo "============================================"

run "health"          GET  "/health"                      "" "ok"
run "service info"    GET  "/"                            "" ""
run "list services"   GET  "/v1/services"                 "" ""
run "auth signup"     POST "/v1/auth/signup"              '{"email":"smoke@test.com","password":"test123","name":"Smoke"}' "" "201"
run "auth login bad"  POST "/v1/auth/login"               '{"email":"x","password":"y"}' "" "201"
run "billing webhook" POST "/v1/billing/webhook"          '{"type":"test"}' "" "201"
