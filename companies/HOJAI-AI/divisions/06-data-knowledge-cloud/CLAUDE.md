...
| **Connector Hub** (8 SaaS adapters) | [./services/connector-hub/](../services/connector-hub/) | **4785** | ✅ NEW (June 20) |
...
| ~~Data Connectors~~ | ~~No pre-built integrations to Salesforce/HubSpot/Stripe/etc.~~ | ✅ DONE (port 4785 — 8 connectors) |
| **Semantic Search** (standalone) | Knowledge Base has it but no dedicated service. | already done |

## 5. Gap Score

**100% of buildable items DONE.** All target capabilities shipped: Vector DB + RAG + Doc Intel + Graph DB + Knowledge Extraction + Connector Hub + Feature Store + Data Catalog.

## 6. Gap List

| # | Item | Status | Notes |
|---|---|---|---|
| 1 | Vector Database + embeddings service | ✅ **DONE** | port 4780 |
| 2 | RAG Platform | ✅ **DONE** | port 4781 |
| 3 | Document Intelligence | ✅ **DONE** | port 4782 |
| 4 | Graph Database | ✅ **DONE** | port 4783 |
| 5 | Knowledge Extraction (NER) | ✅ **DONE** | port 4784 |
| 6 | Data Connectors | ✅ **DONE** | port 4785 — 8 connectors |
| 7 | **Feature Store** | ✅ **DONE** | `services/feature-store` (4164) — ML feature registry with online serving |
| 8 | **Data Lake** | ⚪ **DEPRECATED** | Out of scope — using object storage (S3/GCS) directly via Connector Hub |
| 9 | **ETL pipelines** | ⚪ **DEPRECATED** | Out of scope — Data Connectors + RAG handle this; no separate ETL needed |
| 10 | **Data Catalog** | ✅ **DONE** | `services/data-catalog` (4165) — 180 datasets × 12 industries, searchable |
...
### Connector Hub (port 4785) — ~600 lines — SHIPPED June 20, 2026

Pre-built integrations for 8 external SaaS systems, all behind a unified adapter contract:

| Connector | Kinds |
|-----------|-------|
| **salesforce** | lead, contact, opportunity, account |
| **hubspot** | contact, company, deal, ticket |
| **stripe** | customer, charge, subscription, invoice |
| **shopify** | product, order, customer, inventory |
| **slack** | channel, message, member |
| **notion** | page, database, block |
| **gsheets** | spreadsheet, range |
| **twilio** | message, call |

**Capabilities per connector:** `listResources`, `getResource`, `createResource`, `updateResource`, `deleteResource`, `search`, `sync`. Plus a `connections` registry that stores credentials metadata (never the secrets). 15 demo records seeded across the 8 connectors.

### Connector Hub wired into HOJAI Intelligence (4881)
- `connectorHub: http://localhost:4785` in route table
- New agent `connectorHub` listed in `/api/agents` (37 total after Architecture v2)
- New capabilities: `connectorList`, `connectorResource`, `list-connectors`, `get-connector`, `list-resources`, `get-resource`, `create-resource`, `update-resource`, `delete-resource`, `search`, `sync`, `connection-create`, `connection-list`
...
**Services in this division:** Feature Store, Data Catalog, Knowledge Network, Memory Confidence, Memory Context Engine, RAG, Knowledge Distillation
