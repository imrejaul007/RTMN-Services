 ...
 *
 * Mirrors the pattern in nexha-business-directory and nexha-acp-messaging
 * so all RTMN services have a consistent auth surface. Internal callers
 * MUST supply x-tenant-id (or tenantId in body) because the internal
 * path doesn't imply a tenant — that's intentional.
 *
...
/**
 * Verify a JWT using HS256 + a shared secret (simpler than RS256 + public
 * key for this service since the secret is a shared env var across the
 * internal HOJAI-AI network). For full CorpID RS256 verification, use
 * `@rtmn/shared/auth/createCorpIdAuthMiddleware` instead.
 */
function tryJwt(req) {
  if (!getJwtSecret()) return null;
  const bearer = parseBearer(req);
  if (!bearer) return null;
  // Lightweight base64 decode — avoids pulling in jsonwebtoken as a dep.
  try {
...
export function requireAuth(req, res, next) {
  ...
  if (claims) {
    ...
    if (claims.exp && claims.exp * 1000 < Date.now()) {
      return res.status(401).json({ error: 'Unauthorized', code: 'MARKETPLACE_TOKEN_EXPIRED' });
    }
    req.user = {
  ...
  return res.status(401).json({ error: 'Unauthorized', code: 'MARKETPLACE_AUTH_REQUIRED' });
}
