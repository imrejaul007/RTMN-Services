#!/bin/bash
# sutar-reputation-aggregator - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4258}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_reputation_aggregator_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-reputation-aggregator - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/entities" POST "/api/entities" '{"test":true}' "" "201"
run "auto GET /api/entities/test-id" GET "/api/entities/test-id" "" "" "200"
run "auto POST /api/entities/test-id/scores" POST "/api/entities/test-id/scores" '{"test":true}' "" "201"
run "auto GET /api/leaderboard" GET "/api/leaderboard" "" "" "200"
run "auto POST /api/entities/test-id/sync" POST "/api/entities/test-id/sync" '{"test":true}' "" "201"
