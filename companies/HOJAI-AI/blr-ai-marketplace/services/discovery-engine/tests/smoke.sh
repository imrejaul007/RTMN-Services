#!/bin/bash
# discovery-engine - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4256}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_discovery_engine_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  discovery-engine - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/index" POST "/api/index" '{"test":true}' "" "201"
run "auto POST /api/index/bulk" POST "/api/index/bulk" '{"test":true}' "" "201"
run "auto DELETE /api/index/test-id" DELETE "/api/index/test-id" "" "" "200"
run "auto POST /api/search" POST "/api/search" '{"test":true}' "" "201"
run "auto GET /api/indexes" GET "/api/indexes" "" "" "200"
