#!/bin/bash
# sutar-exploration - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4255}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_exploration_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-exploration - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto GET /api/journeys" GET "/api/journeys" "" "" "200"
run "auto GET /api/journeys/test-id" GET "/api/journeys/test-id" "" "" "200"
run "auto POST /api/journeys/test-id/start" POST "/api/journeys/test-id/start" '{"test":true}' "" "201"
run "auto POST /api/sessions/test-id/step" POST "/api/sessions/test-id/step" '{"test":true}' "" "201"
