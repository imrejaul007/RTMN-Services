#!/bin/bash
# roi-calculator - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4259}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_roi_calculator_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  roi-calculator - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/calculations" POST "/api/calculations" '{"test":true}' "" "201"
run "auto GET /api/calculations/test-id" GET "/api/calculations/test-id" "" "" "200"
run "auto POST /api/calculations/compare" POST "/api/calculations/compare" '{"test":true}' "" "201"
run "auto POST /api/quick-roi" POST "/api/quick-roi" '{"test":true}' "" "201"
