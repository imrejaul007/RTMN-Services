#!/bin/bash
# sutar-multi-agent-evaluator - Auto-generated smoke tests
set -u
BASE_URL="${BASE_URL:-http://localhost:4257}"
PASS=0; FAIL=0
TMPDIR="${TMPDIR:-/tmp}"
...
run() {
  ...
  local body_file="${TMPDIR}/_sutar_multi_agent_evaluator_$$_$RANDOM.json"
  if [ -n "$data" ]; then
...
echo "============================================"
echo "  sutar-multi-agent-evaluator - Smoke"
echo "============================================"

run "auto GET /health" GET "/health" "" "" "200"
run "auto POST /api/evaluations" POST "/api/evaluations" '{"test":true}' "" "201"
run "auto GET /api/evaluations/test-id" GET "/api/evaluations/test-id" "" "" "200"
run "auto POST /api/evaluations/compare" POST "/api/evaluations/compare" '{"test":true}' "" "201"
