...
export class BookingService {
  ...
  static async confirm(params: {
    ...
    // Emit booking.confirmed event to REZ Mind
    rezMindClient.emitBookingConfirmed({
      bookingId: booking.id,
      bookingRef: booking.bookingRef,
      hotelId: booking.hotelId,
      userId: booking.userId,
      ...
      guestPhone: booking.guestPhone,
      totalValuePaise: booking.totalValuePaise,
    }).catch((err) => BookingService.logger.error('[BookingService] REZ Mind booking.confirmed emit failed', { err, bookingId: booking.id }));
